  ...
  res <-
    lapply(workspaces, function(i) {
      ...
      SticsOnR::run_javastics(
        javastics = javastics, workspace = i,
  ...
  for (i in 1:length(workspace_usms)) {
    ...
    for (j in 1:length(parameters_vars)) {
      ...
      if (length(obs_list) == 0) {
        warning(
          "Skipping optimisation of [", paste(parameters_vars[[j]]$params, collapse = ", "),
      ...
      if (length(plant_file) > 1) {
        stop(
          "There must be only one file in the plant folder: ",
...
summarize_optimization <- function(workspaces_orig, workspaces_opti, parameters_vars) {
  ...
  param_orig2 <- get_param_xml(
    file = file.path(workspaces_orig, "param_newform.xml"),
    param = params
  )
  param_optim2 <- get_param_xml(
    file = file.path(workspaces_opti, "param_newform.xml"),
    param = params
  )
