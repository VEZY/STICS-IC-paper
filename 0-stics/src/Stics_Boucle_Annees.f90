...
! 06/01/2017 on calcule un resmes initial avant tout calcul
    ! calcul de la réserve jusqu'à profcalc
      resmes_ini = SUM(sc%hur(1:int(soil%profcalc))+sc%sat(1:int(soil%profcalc)))
      azomes_ini = SUM(soil%nit(1:int(soil%profcalc)))
      write(fichist,*)'initialisations the first day of simulation: ','on the depth ',soil%profcalc,&
         &'initial water content ',resmes_ini, 'initial Nitrogen content ',azomes_ini
...
! DR 10/07/02013 j'ajoute l'ecriture du stade start dans le rapport
    !::: Ecriture des rapports
      ...
      if (iand(pg%P_flagEcriture,sc%ECRITURE_DEBUG)>0)print *,'Stics_Jour: reports'
        ...
        ! domi 24/09/03 pb dans l'ecriture à des stades
          ...
          ! domi - 20/10/03 - je teste pour ne pas ecrire 2 fois le meme jour
            ...
            if ((sc%codetyperap == 2 .or. sc%codetyperap == 3) .and. (sc%ecritrap.eqv..FALSE.)) then
              ...
              if ((n == 1) .and. sc%rapdeb)then
                sc%start_rap=.TRUE.
! on enleve les appel à Mila geres sur la branche Stics_Mila
!                call Ecriture_Rapport(sc,pg,soil,c,sta,p(i),t,patho) ! DR 19/07/2012 on supprime itk qui ne sert pas
...
if (iand(pg%P_flagEcriture,sc%ECRITURE_ECRAN) >0 )write(*,*)'calling of the daily loop Stics_Jour',n,sc%jjul
          ! On appelle la routine journalière de Stics
            ...
            if (iand(pg%P_flagEcriture,sc%ECRITURE_DEBUG)>0)write(sc%ficdbg2,*)'apres stics_jour',n,p(1)%chargefruit&
            ,p(2)%chargefruit
...
!               if (iand(pg%P_flagEcriture,sc%ECRITURE_RAPPORTS) >0)call Ecriture_Rapport(sc,pg,soil,c,sta,p(i),t,patho)
               if (iand(pg%P_flagEcriture,sc%ECRITURE_RAPPORTS) >0)call Ecriture_Rapport(sc,pg,soil,c,sta,p(i),t)
...
!        if (numcult > 1 .and. sc%nbans > 1 .and. sc%P_culturean /= 2) sc%P_iwater = 1
          ...
          if (iand(pg%P_flagEcriture,sc%ECRITURE_ECRAN) >0 )then
                     write(*,*)'numcult nbans culturean iwater',numcult,sc%nbans,sc%P_culturean,sc%P_iwater
          endif
