...
USE Itineraire_Technique
    ...
    call TauxRecouvrement_(sc%n,pg%P_codeinnact,pg%P_codeh2oact,p%turfac(sc%ens),p%innlai(sc%ens),          &
                           ...
                           p%upvt(sc%n),p%upvtutil,p%P_infrecouv,p%densite,p%somcour,                       &
                           ...
                           p%exolai,p%P_stsenlan(itk%P_variete),p%ulai(sc%ens,sc%n-1),sc%tauxcouv(p%nsen),         &
                           p%P_phyllotherme,p%udevcult,                                                     &
                           sc%tustress,sc%tauxcouv(sc%n),sc%tauxcouv(sc%n-1),p%somfeuille,                  &
                           p%nbfeuille,p%reajust,sc%fdens,p%ulai(sc%ens,sc%n),p%efdensite,p%nlan,                  &
                           sc%tauxcouv(p%nlan))
...
! --        if (P_codeperenne == 1 .and. nplt > 0) then
    ...
    ! 2a) calcul de reajust : distance de d�veloppement pour reajuster
...
! ** 4a) calcul du facteur densit� fdens qui joue sur la
...
! --  tauxcouv = P_laiplantule+P_tauxrecouvmax / (1+exp(P_pentrecouv*(infrecouv1-ulai)))
        ...
        ! calcul de l'effet densit� sur le taux de recouvrement
        efdensite = 1.

        ! NB le 22/08/07 bug n-1 au lieu de n car n'�tait pas calcul�
        ...
        ! calcul de la d�riv�e de la logistique
        ar = P_pentrecouv
        br = P_infrecouv
        abr = ar * br
