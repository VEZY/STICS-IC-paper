...
!        P_tigefeuil,P_biorognem,n,nrogne,CNplante,    & !IN DR 20/07/2012 on a plus besoin du Cnplante il est calcul� apres
...
! DR 20/07/2012 on a plus besoin du Cnplante il est calcul� apres
...
!------------------
! DR et ML 18/03/08 :
! on avait supprim� debour et introduit les calculs de debourrement dans
! develop apres reflexion ca pose des pbs car le basculement du calcul horaire au calcul
! journalier dependait de la lev�e qui etait calcul� apres le test de basculement
! d'ou des somtemps horaires le jour de la lev�e qui entrainait une scenescence
...
call develop2(c%phoi,c%tmax(sc%n),c%tmin(sc%n),c%tmin(sc%n+1),c%trr(sc%n),g%P_codeh2oact,g%P_codeinitprec,&
              ...
              sc%nbjsemis,sc%numcult,sc%tairveille,sc%tcult,sc%tsol,sc%xmlch1,t%P_codepluiepoquet,t%P_codetempfauche,t%humectation,&
              t%nbjhumec,t%pluiesemis,t%somtemphumec(p%ipl),p%P_codeindetermin,p%P_codelaitr,p%P_codlainet,&
...
!              p%namfobs,itk%P_nbcueille,p%P_nboite,p%nfloobs,p%nlanobs,p%nlaxobs,p%nmatobs,p%nrecobs,p%nsenobs,p%P_stdrpnou,       &
              ...
              p%somcourfauche,p%somcourutp,p%somtemp,p%stpltlev,p%tdevelop(sc%n),p%udevair,p%udevcult,p%upvt(sc%n),p%utp(sc%n), &
              ...
              ! 23/07/2012 hilueder et sucreder inutilis�s
...
!              p%jdepuisrec,p%pdsfruit(0,p%P_nboite),p%nbrecolte,p%nrecint(p%nbrecolte),p%rdtint               ,                   &
              p%teauint(0,p%nbrecolte),p%nbfrint(0,p%nbrecolte),t%onestan2,p%somcourmont,t%nmontaison(p%ipl),p%stpltger,          &
                ! DR 06/03/2015 ajout pour les enchainement annuel de la vigne
