...
! *-----------------------------------------------------------------------------------------------------------------------------------------------------------* c
!subroutine calai(sc,pg,p,itk,t)  !DR 19/07/2012 t n'est pas utile car codedlaimin a ete supprim�
...
!write(618,*)'jour',n,'ens',ens
    call calai_(pg%P_codeinnact,pg%P_codeh2oact,p%P_codehypo,p%P_codegermin,itk%P_codcueille, & ! IN
                ...
                p%namf,p%nlax,p%ndrp,p%nrec,p%upvt(n),p%upvtutil,p%P_vlaimax,                 &
                ...
                p%fpv(ens,n),p%P_stlaxsen(itk%P_variete),p%tempeff,p%lai(ens,:),              &
                p%somfeuille,p%nbfeuille,p%nsen,p%nlan,p%P_dlaimax,p%reajust,p%ulai(ens,:),   &
                p%vmax(ens),p%dltaisenat(ens),p%laisen(ens,:),p%dltaisen(ens),                    &
                p%P_stsenlan(itk%P_variete),p%densiteequiv,sc%lai_tot_prev)
...
!: cumuls AO/AS
    ...
    p%fpv(sc%AOAS,n) = p%fpv(sc%AS,n) * p%surf(sc%AS)+ p%fpv(sc%AO,n) * p%surf(sc%AO)

    !p%lai(sc%AOAS,n) = p%lai(sc%AS,n) * p%surf(sc%AS) + p%lai(sc%AO,n) * p%surf(sc%AO)
    ! remplac� par : (extrait de cumAOetAS.f90)
    ...
    if (lge(sc%P_codesimul,'feuille') .eqv. .FALSE.) then
      ...
      p%dltaisen(sc%AOAS) = p%dltaisen(sc%AS) + p%dltaisen(sc%AO)
...
subroutine calai_(P_codeinnact,P_codeh2oact,P_codehypo,P_codegermin,P_codcueille,P_codlainet,codeulaivernal,      & !IN
                  ...
                  dltaisen,P_stsenlan,densiteequiv,lai_tot_prev)
...
real,    intent(IN)    :: densiteequiv  !densite equivalente calcul�e chaque jour
real,    intent(IN)    :: lai_tot_prev ! Used to compute a plot scale lai from previous day to compute lai_comp for intercrops
...
!-------------------------------------------------
    if (nlev == 0) then
      ...
      if (P_codeperenne == 1 .and. n >= nplt) then  !- NPLT �tant non nul d�s le depart de la simulation, on test n>=nplt
        ...
        lai(n) = 0.0
        return
        !: TODO - le somfeuille=0 apr�s le return est inutile, il n'est jamais ex�cut�. BUG ?
        somfeuille = 0.
      endif
    endif

    ! le jour de la lev�e, si plantation par plantule, on force le lai du jour pr�c�dent
    ...
    if (nrec > 0 .and. n > nrec) then
      ! En cas de moisson, on enl�ve les feuilles.
...
! write(618,*)'lai avant deltai',n , lai(n-1), lai(n)
    ...
    !: si option LAI brut+ s�nescence
    if (P_codlainet == 2) P_dlaimax = P_dlaimaxbrut

    !: 2a) calcul de reajust : distance de d�veloppement pour r�ajuster
    !-     les parcours observ�s
    ...
    !: 2b) lai de d�part non nul pour les cultures qui se plantent
    !--      if (n == nlev .and. P_codehypo == 2 .and. namf == 0) then
    !--        lai(n-1) = P_laiplantule
    !--!: NB - le 21/04 - sbv remplac� par slavert
...
!write(618,*)'ulai 6',n,nlev,namf,P_vlaimax,somcour,reajust,P_stlevamf,ulai(n)
    ...
    !: 3c) ulai maximal atteint � drp pour les ind�termin�es
...
!--------------------------
    ...
    !: 4a) calcul du facteur densit� efdensite actif � partir de P_laicomp
    !- calcul de l'effet densit� sur la mise en place du LAI pour les stics-plante
    efdensite = 1.
    if (ulai(n) > 1.) then
      if (lai_tot_prev < P_laicomp) then ! using lai_tot_prev instead of just lai to consider the other species too
        efdensite = 1.
      else
        if (densite == 0) then
          efdensite = 0.0
        else
          efdensite = min(1.0,(densiteequiv / P_bdens)**P_adens)
        endif
      endif
    else
    ...
    ! ** 4b) temp�rature efficace de croissance: limitation par P_tcmax
    ! NB le 13/06/06
    ! introduction d'une temp�rature maximale d'arr�t de croissance
    ! foliaire P_Tcxstop � l'occasion �tude Fran�oise sur fourrages m�diterran�ens
    !      P_tcxstop = 30.0
    ...
    !-  * la croissance s'arr�te � LAX pour les d�termin�es
    !-  * la croissance s'arr�te � SEN pour les ind�termin�es
    ...
    if (nstopfeuille == 0) then
      ...
      if (ulai(n) <= P_udlaimax .or. P_udlaimax == 3.0) then
        ...
        ! introduction d'une ordonn�e � l'origine pour la croissance des feuilles
...
!        endif
        ...
        !: entre P_udlaimax = 1 et ulai = 3(lax), la vitesse est d�croissante
        !- sauf pour les ind�termin�es
...
!        deltai = vmax * (1 - (ulai(n) - P_udlaimax) / ((3.0 - P_udlaimax))**2)
      ...
      !: NB - le 28/05 - on l�ve toujours
      !- 10/01 - ajout d'un test sur l'enchainement des perennes
      ! TODO: voir si on pourrait remplacer ce test par un bool�en qui dirait si la plante a lev� ou pas. ex: *if (isLevee) then*
...
!     write(*,*)P_codedlaimin, P_codeindetermin
      ...
      !: la croissance foliaire des plantes ind�termin�es d�pend
      !- du rapport source/puits des organes v�g�tatifs
      if (P_codeindetermin == 2) then
        ...
        ! NB le 21/04 recalcul d'un sbv appropri�
       ...
       ! write(618,*)'3deltai,  splai',deltai,  splai
        ...
        !- apr�s AMF (cas des cultures associ�es)
        !- NB le 21/04 recalcul d'un sbv appropri�
        !- NB le 15/05/02 calcul de fpv pour les p�rennes
        sbvmin = P_slamin / (1.0 + P_tigefeuil)
        fpv = deltai * 1e4 / sbvmin
        sbvmax = P_slamax / (1.0 + P_tigefeuil)
        !: ajout des remobilisations possible pour faire red�marrer le
        !- lai en cas de couvert compl�tement sec
        !- NB le 13/06/06
        !- NB le 27/06/06 attention delatremobil �tait d�j� compt� dans dltams
...
!: 5. PHASE DE PLATEAU  (si option lainet et culture determin�e)
!---------------------------------------------------------------
...
!: 6. Calcul de la senescence rapide par ajustement a la r�colte
...
!---------------------------------------------------------------
    ...
    !  write(618,*)'4 lai(n),lai(n-1) , deltai , dltaisenat',lai(n),lai(n-1) , deltai , dltaisenat
      ...
      !: option LAI brut la s�nescence (dltaisen) est calcul� dans le spg senescen
    ...
    ! NB - le 22/04 - � cause pr�cisison dans les soustractions
...
!: 7. senescence foliaire due � des stress  pour l'option LAInet
!---------------------------------------------------------------
    ...
    if (P_codlainet == 1 .and. lai(n) > 0.0) then
      ...
      if (tustress < P_tustressmin .or. tcult < P_tcmin .or. fstressgel < 1.0) then
        ! bug dans l'op�ration NB le 08/05
