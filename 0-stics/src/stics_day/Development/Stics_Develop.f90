...
! DR 23/07/2012 latitude est pas itilis� car on passe la photoperiode qui est suffisante
...
subroutine develop2(phoi,tmax,tmin,tmin_demain,trr,P_codeh2oact,P_codeinitprec,P_codeinnact,codeulaivernal,P_psihucc, &
                    ...
                    ! DR 23/07/2012 P_nboite non utilis�
...
! dr 23/07/2012 huileder et sucreder inutilis�s
...
!  real,    intent(IN)                           :: P_latitude  ! // PARAMETER // Latitudinal position of the crop  // degree // STATION // 0
  ...
  real,    intent(IN)                           :: innlai   ! // OUTPUT // Index of nitrogen stress active on leaf growth // P_innmin � 1
  integer,    intent(IN)                           :: P_julvernal  ! // PARAMETER // julian day (between 1 and 365) accounting for the beginning of vernalisation for perennial crops // julian day // PARPLT // 1
...
! DR 06/03/2015 ajout pour les enchainement annuel de la vigne
  ...
  real,    intent(OUT)                          :: P_stflodrp  ! // PARAMETER // phasic duration between FLO and DRP (only for indication) // degr�s.jours // PARPLT // 1
...
!! dr 22/12/2010 on a ajout� la dimension
...
!: Variables locales
  ...
  !n = sc%n ! juste pour que �a soit plus facile � �crire
...
!: Pour les annuelles => pas de d�veloppement avant le semis
!- DR - 03/05/06
...
!: On remet � z�ro les masses s�ches, masses fraiches, masses des fruits, la hauteur de la plante, les variables de fixation (et le zrac ?)
!: Marie et Domi - 10/10/03 - bug signal� par Emmanuelle Sauboua
!- N'�tait jamais test� dans 'recolte'
  if (P_codeperenne == 1 .and. P_codcueille == 1) then
    if (n == nrec+1) then
      ...
      demande(:)        = 0. ! PB - 03/05/2004 - remise � z�ro des variables de fixation
...
!: Pour les cultures annuelles (P_codeperenne=1) moissonn�es (P_codcueille=1) : pas de d�veloppement apr�s la r�colte.
    if (n > nrec .and. nlan > 0 .and. nrec > 0) return
  endif

!: PB - 11/03/2005 - On remet pdsfruitfrais � z�ro apr�s la r�colte.
!: PB - 07/08/2009 - Attention, qd n=1 et nrec=0, cette condition peut �tre vraie alors qu'on est pas apr�s la r�colte.
...
! -------------------------------------------
  ...
  !: Unit�s journali�res
  !- NB - 07/07/06: Application du seuil thermique n�gatif P_TCXSTOP au d�veloppement
  if (P_codegdh == 1) then
  ...
  if (P_codegdh == 2) then
    ...
    !: 1) Reconstitution des temp�ratures horaires.
    !-    Pour l'instant on n'autorise uniquement le cas par temp�ratures air.
...
!-- si cette phase n'est pas realis�e (fin de dormance) on ne cumule rien
!--    if (nfindorm == 0) then
!-- maintenant qu'on a tout remis d'equerre avec debour ca ne sert � rien
!--    if (nfindorm0 == 0) then
!--    udevcult=0.0
!--    udevair=0.0
!--    endif
  ...
  ! NB - le 01/09/06:
  ! effet retard du stress hydrique en phase v�g�tative appliqu� directement
  ! sur les udevair (ou udevcult) de fa�on � ce qu'il agisse sur le d�veloppement
  ! P_ph�nologique et �galement sur la dur�e de vie.
...
!    endif
!  endif
  ! Modif Bug Lo�c Ao�t 2017 : probl�me lorsque l'on activait un seul stress on prenait
  ! quand m�me le minimum des deux stress...???!!!
  if (P_coderetflo == 1 .and. (P_codeinnact == 1 .or. P_codeh2oact == 1 )) then
      if (P_codeplante == 'qui' .or. ndrp == 0) then
             ...
             if (P_codetemp == 2) then
               ...
               udevair = udevair * (P_stressdev * min(turfac,innlai) + 1 - P_stressdev)
                ...
                if (P_codeinnact == 1 .and. P_codeh2oact == 2 ) then
                  if (P_codetemp == 2) then
                     udevcult = udevcult * (P_stressdev * innlai + 1 - P_stressdev)
                  else
                     udevair = udevair * (P_stressdev * innlai + 1 - P_stressdev)
                  endif
                endif
                if (P_codeinnact == 2 .and. P_codeh2oact == 1 ) then
                  if (P_codetemp == 2) then
                     udevcult = udevcult * (P_stressdev * turfac + 1 - P_stressdev)
                  else
                     udevair = udevair * (P_stressdev * turfac + 1 - P_stressdev)
                  endif
                endif
          endif
       endif
...
! ---------------------------------
  ...
  !- ou en cours de culture apr�s la date P_julvernal
  !- calculs donn�es interm�diaires
  if (P_codebfroid == 2) then
    !: Choix de la temp�rature (tdev) pour les besoins en froid
    if (P_codetemp == 2) then
      tdev = tcult
    else
      tdev = tairveille
...
! -----------------------------
  if (P_codephot == 1) then
    ...
    if (ndrp /= 0 .or. n > ndrpobs) then
      rfpi = 1.0
    else if (P_codebfroid /= 3 .and. (nlev == 0 .or. n < nlev)) then ! pour les ligneux, photop�riode active � partir de la fin de dormance
      rfpi = 1.0
    else if (P_codebfroid == 3 .and. nfindorm == 0) then
      rfpi = 1.0
    else
      ! TODO: on remplace l'appel � photpd par ses r�sultats. photpd n'est appel� qu'une fois par pas de temps
...
! ---------------------------------------------------
  if (P_codetemp == 2) then
    ...
    ! implicitement n'est utilis� que si on est en enchainement
    !! DR et ML 26/09/2013 il manquait le test sur codebfroid si bien que le ble passait par la ligne et ne faisait de vernalisation apres lev�e
    !! DR et ML et IGCA 26/09/2013 on enleve le test sur la dormance qui ne sert � rien la (on cumule du chaud)
!    if (P_codebfroid == 3 .and. P_codedormance == 3 .and. nlev > 0) then
  ...
  !: somcour = cumul d'unit� entre deux stades v�g�tatifs
  somcour = somcour + upvt

  !: somcourdrp = cumul d'unit� entre deux stades reproducteurs
  !- � partir de la levee
  ...
  !: calcul d'une somme de temp�ratures m�me
  !- pour les plantes vernalo-photo-sensibles
  !- pour les calculs de s�nescence et de nombre  de feuilles
  !-
  !- NB le 08/05 on remplace la somme des temp�ratures pour
  !- la s�nescence par un P_Q10 pour que le vieillissement soit effectif meme
...
! ####
! entr�e en vernalisation des fourrages (p�renne)
! NB le 07/03/08
    if (P_codebfroid == 2 .and. P_codeperenne == 2) then
      somcourmont = somcourmont + upvt
    ! PB - 03/08/2010 - je remplace jul par jjul qui correspond � n+P_iwater-1
      if (jjul+((onestan2-1)*nbjsemis) == P_julvernal) then
        somcourmont = 0.0
      endif
! ** stade d�but montaison : apr�s vernalisation
! unique cycle reproducteur de l'ann�e
      if (somcourmont >= P_stlevamf .and. jjul+((onestan2-1)*nbjsemis) > P_julvernal)then
        nmontaison = n
        namf = n
! DR et ML et SYL 16/06/09 - on supprime nvernal qui ne sert � rien
!--         nvernal=0
        somcourmont=0.0
        onestan2 = 1
      endif

! si la coupe intervient apr�s le stade montaison alors on remet
! le stade � 0
...
! DR 18/07/2012 je rajoute la germination
    if (n  == nger) then   ! si on est le jour de la lev�e
      stpltger = somcour ! on affecte le cumul de temp�rature entre le semis et la lev�e
! DR 18/07/2012 pour la germination on affiche juste somcour sans le reinitialiser
!      somcour = 0.0        ! on remet � z�ro le cumul de temp�rature courant.
!      if (P_codeperenne == 1) somcourdrp = 0.0 ! NB le 23/03 pour les p�renne d�but du d�compte drp � la lev�e de dormance
!      if (P_codefauche == 2) somcourfauche = 0.0
!      if (codeulaivernal == 0) somcourutp = 0.0
    endif


  if (nlevobs == 999) then ! pas d'observation pour la lev�e
    if (n  == nlev) then   ! si on est le jour de la lev�e
      stpltlev = somcour ! on affecte le cumul de temp�rature entre le semis et la lev�e
      somcour = 0.0        ! on remet � z�ro le cumul de temp�rature courant.
      if (P_codeperenne == 1) somcourdrp = 0.0 ! NB le 23/03 pour les p�renne d�but du d�compte drp � la lev�e de dormance
      if (P_codefauche == 2) somcourfauche = 0.0
      if (codeulaivernal == 0) somcourutp = 0.0
    ...
    if (n  == nlevobs) then  ! si on est le jour de la lev�e observ�e
      nlev = nlevobs         ! on force nlev
      if (nger <= 0) nger = nlev  ! si la germination n'a pas encore �t� affect�e, on la force au jour de la lev�e
      ! r�ajustement du parcours de dl
      stpltlev = somcour       ! on affecte le cumul de temp�rature entre le semis et la lev�e
      somcour = 0.0            ! on remet � z�ro le cumul de temp�rature courant.'
      if (P_codeperenne == 1) somcourdrp = 0.0 ! NB le 23/03 pour les p�renne d�but du d�compte drp � la lev�e de dormance
      if (P_codefauche == 2) somcourfauche = 0.0
...
!: STADES REPRODUCTEURS
  ...
  if (nfloobs == 999) then
    if (somcourdrp >= stlevflo .and. nflo == 0) then
      nflo = n
      stlevflo = somcourdrp
!--      somcourdrp = 0.0 ! domi 04/04/01  pb canne on supprime la remise � zero
    endif
  else
    if (n == nfloobs) then
      nflo = nfloobs
...
! lorsque on force flo ou drp => 2 modifs on enleve la remise � zero de somcourdrp si
...
!--      endif
  ...
  if (nmatobs == 999) then
    if (P_codeindetermin == 1) then
      ...
      ! pour les ind�termin�es la maturit� finale correspond � l'ensemble des P_nboite-1 vides
      ! Nb le 01/05 si P_nbcueille = 1
      ! si P_nbcueille = 2 : la maturit� correspond au d�but de remplissage de la derni�re boite
      if (P_nbcueille == 1) then
!--            if (nbfruit == 0.0 .and. nmat == 0 .and.
        ! 12/07/06  DR et IGC nous avons chang� la condition de calcul de la date
        ! de maturit�. Avant il calculait celle-ci en fonction du nombre de grains.
        ! Maintenant, on calcule en focntion de la dur�e de fruits.
  ...
  !: stade rec
  if (nrecobs == 999) then
    ! d�temination de la date de r�colte par la teneur en eau des grains � partir de la maturit�
...
!--     endif
    ...
    !: R�colte
    !- PB - 18/01/2005 - pas de r�colte qd culture fauch�e.
    if (P_codefauche /= 1) &
...
!  P_huilerec,sucre,huile,teaugrain,sucreder,huileder,P_h2ofrvert,P_codeaumin,    & ! 23/07/2012 hileder et sucreder non utilis�s
                    P_huilerec,sucre,huile,teaugrain,P_h2ofrvert,P_codeaumin,     & ! 23/07/2012 hileder et sucreder non utilis�s
...
!  write(*,*)'** dans develop apres recolte ',rdtint,nbrecolte
    ...
    if (n == nrecobs) then
      ...
      if (nrec < nmat .or. nmat == 0) then
        ...
        ! pour la vigne on peut recolter meme si on est pas � mat
...
!: Affectation des bonnes valeurs d'unit�s de d�veloppement
!- utilis�es pour le calcul du LAI
  upvtutil = upvt
...
if(n >= nrecbutoir .and. P_codeplante.eq.'fou'.and. P_codeinitprec == 2)then
  ! write(*,*)'on est � la fin mais on met pas nerecbutoir'
else
...
! NB et IG le 23/09/06 suppression du test pour les cultures sur plusieurs ann�es
!      if (n == nrecbutoir) then
    if (nrec == 0) then
      group = -1
      nrec = nrecbutoir
...
! NB le 25/08/04 traitement debdes en cas de recolte butoir
    if (ndebdes == 0) then
! dr 01/12/2014 si le stade ndebdes n'est pas realis� on le force plutot � recolte
!      ndebdes = nrecbutoir
      ndebdes = nrec
...
!
    if (ndrp > 0 .and. nmat == 0) then
      nmat = nrecbutoir
! NB le 25/08/04 r�activation ligne suivante
      P_stdrpmat = somcourdrp
    endif
  endif
endif
...
!--    if (P_stade0.ne.'snu'.and.P_stade0.ne.'plt'.and.P_stade0.ne.'lev') then
    ...
    !: entr�e en vernalisation des herbac�es p�rennes au jour P_julvernal
...
!--    endif
    ...
    if (.not.etatvernal) then
      ...
      !: On active P_julvernal pour les annuelles, qui joue lorqu'il se situe apr�s la germination.
      !- NB et ML - 25/02/04 : � tester sur RGI CIPAN
      !- ML - 21/04/04 : on interdit de d�marrer au stade 'dor' et d'avoir jul < P_julvernal pour les cultures perennes
      if (nger /= 0.) then
      ...
      ! ####
       ...
       ! ####
          ...
          !: ML - 28/05/04 : on interdit de d�marrer au stade lev et
          !- d'avoir jul > P_julvernal le jour de la lev�e (d�but de vernalisation) pour les cultures perennes
...
!-- domi 21/07/04 inclusion des modifs de marie du 28/05/04 version 52
!--                call bfroid(tdev)
!--              else
!--                if (P_codeperenne == 1) then
!--                  rfvi=1
...
!          ndebdorm = 213 + 365 - 346 + 1
          ...
          !: DR - 20/11/06 : On est un peu perplexe , on etait sur d'avoir test� tous les cas.
          !- Quand on arrive en fin de dormance, si on ne met pas nlev=0 on ne calcule plus les sommes
          !- d'action chaude de richardson. La date de lev�e a �t� stock�e dans ilevs
          nlev = 0
        else
          rfvi = 0.0
        end if
        ...
        !: Cas de for�age de la lev�e de dormance
        if (n < nfindorm) then
         rfvi = 0.0
        else
          rfvi = 1.0
      ...
      !: ML - le 18/10/05 : Cas de la dormance calculee avec Bidabe: on demarre la dormance � ndebdorm
