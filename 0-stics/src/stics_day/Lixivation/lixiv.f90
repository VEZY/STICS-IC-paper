...
subroutine lixiv(n,precip,hurlim,ruisselsurf,humin,nhe,nh,hucc,hr,P_codefente,ncel,icel,da,&  ! les entrées
                 ...
                 profsem,P_profmesW, P_profmesN, SoilAvW, SoilWatM, SoilNM, lessiv,som_HUR,som_sat) ! ajout entrees et des sorties Macsur
...
! dr 16/12/2013 ajout pour Macsur
  ...
  real,    intent(OUT)   :: som_HUR
  real,    intent(OUT)   :: som_sat
...
! dr 26/03/2014 reactivation , la variable n'etait plus calculée
    ...
    ! DR 06/01/2017 on ajoute 2 varaibles pour verifier les bilans d'eau
      som_HUR=SUM(hur(1:int(profcalc)))
      som_sat=SUM(sat(1:int(profcalc)))
