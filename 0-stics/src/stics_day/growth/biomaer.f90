...
! *-  int�gration des r�serves  hivernales
! *-  P_remobres = proportion de r�serves mobilisables
! *-             par rapport � la croissance
! *-             � relier � la quantit� totale de r�serve
! *-  P_resperenne0 = r�serves en t/ha
...
subroutine biomaer(n, ipl, ens, nlev, P_codeperenne, nplt, P_codehypo, P_codegermin, P_masecplantule, &
                   ...
                   densiteequiv, P_codeh2oact, swfac, exobiom, P_codeinnact, dltaremobil, &
                   P_codeindetermin, fpv, fpft, &
                   P_remobres, P_resperenne0, demande, P_QNplante0, P_msresiduel, P_nbcueille, rdtint, &
                   CNgrain, P_codemontaison, nmontaison, masec_veille, masec, QNplante_veille, QNplante, &
                   inns, inn, innlai, cumdltaremobilN, ebmax, ftemp, epsib, fco2, dltams, dltamsen, &
                   dltamstombe, resperenne, dltamsN, photnet, sourcepuits, dltaremobilN, remobilj, &
                   cumdltares, magrain_veille, magrain, masecneo, surface, surfaceSous, P_nbplantes, &
                   P_extin, P_extin2, cumrg, cumraint, fapar, P_adfol, lairognecum, laieffcum, &
                   P_dfolbas, &
                   P_dfolhaut, dfol, rdif, parapluie, raint, P_parsurrg, P_forme, lai, lai2, laisen, eai, eai2, &
                   P_interrang, nlax, nsen, P_codlainet, P_hautbase, P_codepalissage, P_hautmaxtec, &
                   P_largtec, originehaut, hauteur, deltahauteur, P_hautmax, varrapforme, largeur, jul, &
                   trg, P_latitude, rombre, rsoleil, P_orientrang, P_ktrou, tauxcouv, P_khaut, surfAO, &
                   surfAS, fpari, P_masec0, P_hautK, P_hautA, P_code_shape, nflo, nmat, &
                   P_stage_const_height, P_elongation, P_hautdens, ef_elongation, true_dominance, &
                   P_haut_dev_x0, P_haut_dev_k, somupvtsem, turfac, P_nw_height, ef_n_w_height, &
...
! IN
   ...
   real, intent(IN)    :: densiteequiv   ! // OUTPUT // Equivalent plant density in sole crop (for intercrop) // plants.m-2
   ...
   real, intent(IN)    :: P_masec0
   real, intent(IN)    :: turfac   ! // OUTPUT // Index of turgescence water stress  // 0-1

! (IN)OUT
   real, intent(INOUT) :: masec_veille    ! masec_veille => valeur pr�c�dente (valeur de la veille par ex.)
   real, intent(INOUT) :: masec           ! masec => valeur actuelle (valeur du jour courant par ex.)    // OUTPUT // Aboveground dry matter  // t.ha-1
   real, intent(INOUT) :: QNplante_veille ! QNplante_veille => valeur pr�c�dente (valeur de la veille par ex.)
   ...
   real, intent(INOUT) :: innlai   ! // OUTPUT // Index of nitrogen stress active on leaf growth // P_innmin � 1
   ...
   real, intent(INOUT) :: magrain_veille  ! magrain_veille => valeur pr�c�dente (valeur de la veille par ex.)
...
!  integer, intent(IN)    :: P_codetransrad
   ...
   real, intent(INOUT) :: P_extin2  ! // PARAMETER // extinction coefficient of photosynthetic active radiation canopy of the other plant if any// SD // PARPLT // 1
...
! TRANSRAD
   ...
   real, intent(IN)    :: lai2   ! // OUTPUT // Leaf area index (table) of the other plant if any // m2 leafs  m-2 soil
...
! -- FIN BEER
   ...
   integer, intent(IN)    :: P_nrow ! // PARAMETER // number of plant rows in a strip
...
! ####
! *- SYL 190209 - supression de la d�claration de P_resplmax qui a �t� introduit dans le paramV6
!   real P_resplmax
! ####
...
!les FONCTIONs
   real :: calculerEffetTemperature ! � mettre dans le module Divers ?

! 1.
   !: Pas de biomasse avant la lev�e
   ...
   if (nlev == 0) then
      ...
      if (P_codeperenne == 1 .and. n >= nplt) then
         ...
         if (P_codehypo == 2 .and. P_codegermin == 1) then
            ...
            ! NB le 13/07/05 remplacement de P_adilmax par P_adil pour homog�n�it� avec P_QNplante0
            ! DR le 10/11/05 du fait que dans stressN on calcule QNPlante � partir du QNPlante du jour -1 on a un souci
   ...
   if (nrec > 0 .and. n > nrec) then
      !: En cas de moisson, on enl�ve toute la biomasse a�rienne
...
! A REGARDER PAR NADINE
   ...
   !: NB le 01/05 - il faut passer dans raytrans pour acc�der � la routine "beer"
   !- TODO: rassembler dans initial tous les codes impos�s quand P_codelaitr = 2
   ...
   !      de v�rification, qui enverrait un message � l'utilisateur pour le pr�venir qu'en l'�tat actuel du code
   !      on force une loi de beer dans le cas du taux de couverture.
   !      Ce test pourrait �tre d�port� dans raytrans d'ailleurs plutot qu'ici.

! 2. le rayonnement intercept�
   call raytrans(P_codetransrad, P_extin, P_extin2, cumrg, cumraint, P_adfol, lairognecum, laieffcum, P_dfolbas, &
             P_dfolhaut, dfol, rdif, parapluie, raint, P_parsurrg, P_forme, lai, lai2, laisen, eai, eai2, P_interrang, nlax, nsen, &
                 P_codlainet, P_hautbase, P_codepalissage, P_hautmaxtec, P_largtec, originehaut, hauteur, deltahauteur, P_hautmax, &
               varrapforme, largeur, jul, trg, P_latitude, rombre, rsoleil, P_orientrang, P_ktrou, P_codelaitr, P_khaut, tauxcouv, &
               surface, surfaceSous, ipl, P_nbplantes, ens, surfAO, surfAS, P_hautK, P_hautA, P_code_shape, masec_veille, densite, &
                 nflo, ndrp, nmat, nrec, P_stage_const_height, P_elongation, P_hautdens, ef_elongation, true_dominance, &
                 P_haut_dev_x0, P_haut_dev_k, somupvtsem, inns, turfac, P_nw_height, ef_n_w_height, potential_height, &
                 fstressgel, exolai, P_code_strip, P_nrow, light_beer_active)

! 3. Machi�re s�che potentielle
! ** sa conversion
...
! NB le 23/06/06
! apr�s r�colte chez les p�rennes on revient � une efficience v�g�tative
   if (ndrp /= 0 .and. chargefruit <= 0.0) ebmax = P_efcroiveg/100.

   !: Calcul de l'effet rayonnement
   fpari = ebmax - (P_coefb*raint*(densiteequiv/densite))
   ! RV: (densiteequiv/densite) is used to retreive an equivalent raint as
   ! in sole crop (=1 for sole crop) because P_coefb is given for sole crop

! ** calcul de l'effet temp�rature ftemp
...
! *- modifi� le 19/01/02
   ...
   dltams = epsib*raint
! 4. Mati�re s�che r�elle

! NB et IG le 22/09/06 limitation des r�serves : effet puits sur les sources
! DR et ML et SYL 15/06/09
! **************************
! introduction de la fin des modifications de Sylvain (nadine et FR)
! dans le cadre du projet PERMED
...
! ** effet des stress
! *- Nb - le 07/06 - introduction de l'exc�s d'eau
! *- les3 29/04/03  bug (exobiom jouait au carr�)
   ...
   dltamsN = dltams
! BM, NB le 11/04/05
! d�placement de l'influence du stress azot� pour le calcul de dltamsN
...
! ** mobilisation des r�serves
   dltams = dltams + dltaremobil
! ** recalcul de l'epsilon b et changement d'unit�
...
! ** remobilisation des r�serves chez les chez les p�rennes
! *- ou chez les ind�termin�es (pour remplacer le param�trage
! *- de remobilj
...
! ** calcul du rapport source/puits
      ...
      !: Calcul des r�serves mobilisables pour les p�rennes
...
!          write(73,*) remob,P_remobres,resperenne
         if (cumdltares < P_resperenne0) then
            ...
            !: Remobilisations d�j� compt�es dans l'accumulation de biomasse
            !- utilis�e dans fruit.for pour calculer le rapport sourcepuits
            remobilj = min(remob, resperenne)

            !: NB et ML le 10/10/09: correction: on ne soustrait pas remobilj � resperenne
      ...
      !: Cr�ation d'une variable de cumul de remobilisation
...
!  endif
   ...
   !: Pour les cultures fauch�es demarrage
   !- � la matiere seche residuelle de la coupe precedente
   if (n == nlev .and. P_codefauche == 1) then
      ! 12/01/2017 dans le cas des enchainements c'est le masec0 qui est soit lu dans le recup en cas d'enchainement, soit celui du ini
      ! masec_veille = P_msresiduel
      masec_veille = P_masec0
   end if

   masec = masec_veille + dltams - dltamstombe

!   write(*,*)n,masec_veille, dltams , dltamstombe,masec
   !: Domi 06/06/01  mati�re s�che < 0
   if (masec < 0.) then
      dltamsen = 0.
      dltamstombe = 0.
      masec = 0.
...
!DR attention, 05/12/2014 on l'enleve apres dans cumaoas
         masec = masec - rdtote
!          write(*,*)'nrec dans biomaer',masec,rdtote,P_nbcueille,rdtint,magrain_veille
         !: NB&PB - 22/02/2005 - on enl�ve l'azote des fruits
         QNplante = QNplante - (CNgrain*rdtote*10)
      end if
   end if

! ** TODO : voir si on peut supprimer ces commentaires.
! ** cumul r�el de la mati�re s�che � partir du seuil MSAER0
! *- proposition de F. Ruget et X. Tayot pour tenir compte de
! *- l'allocation pr�f�rentielle aux racines en d�but de cycle
! *- ajout de la condition sur P_masec0 pour cultures plant�es
...
! --    if (naer == 0) then
! --      masec = 0.0
! -- ** si la biomasse est inf�rieure � MSAER0 � la r�colte
