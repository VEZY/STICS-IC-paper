...
! III. Nouveau mod�le de croissance racinaire (Devienne et al., mars98)
! *********************************************************************
      else
        if (nger > 0) then
...
! print *,'avant densiteVraieRacinaire'
          call densiteVraieRacinaire(n,nbCouches,dacouche,hur,humin,anox,nstoprac,P_codazorac,P_minazorac,P_maxazorac,P_minefnra,&
                                     ...
                                     humirac_z,efnrac_z,rlj,masec,P_codemortalracine,dltmsrac_plante,densiteequiv)

! print *,'apres densiteVraieRacinaire'
!#if DEBUG == 1
...
! en d�cadaire,  initialisation de la croissance racinaire � la lev�e
!
! cas d'une prairie : si la prairie est installee codeinstal = 1
! si on est � la deuxieme coupe, on ne passe pas dans croira
...
!- NB et ML - le 23/02/2004
      ...
      if (n == nger .and. codeinstal == 0) then
        if (P_codehypo == 2) then
          ...
          !: Initialisation de la croissance racinaire � la germination
          znonli = P_profsem
          zrac = znonli
        endif
      endif
...
! DR et ML et SYL 15/06/09 FIN introduction de la fin des modifications de Sylvain
    ...
    ! (nstoprac == 0 .or. n == nstoprac) de fa�on � ce que l'activit� des
    ! nodosit�s se poursuive apr�s l'arr�t de la croissance racinaire
      if (n >= nger .and. nger > 0 .and. zrac > 0.) then
        ...
        ! dans le cadre du projet PERMED
          if (P_codedyntalle == 1) then
        ! ####
        ! NB le 06/03/2008 introduction de l'effet n�gatif des temp�ratures �lev�es
...
! 15/06/09 on introduit cette modif conditionnelle � P_codedyntalle
...
! ** croissance racinaire de la germination jusqu'� la floraison
! *- PB - 29/04/2004 - rajout d'un test si zrac plus grand que z�ro
      if (n >= nger .and. (nstoprac == 0.or.n == nstoprac) .and. nger > 0 .and. zrac > 0.) then
! *- calcul de l'effet temp�rature pond�r� par la s�cheresse du sol
...
! *- introduction d'une sensibilit� � l'anoxie : P_sensanox NB le 16/1/2001
...
! ** fonction humidit� en tout ou rien de part et d'autre du PF 4.2
! *- le front racinaire est davantage ralenti au stade radicelle (avant la lev�e)
! *- NB - le 18/12/01
        if (nlev == 0) then
...
! DR 11/06/2013 on sort la variable de sortie de humirac apres lev�e
        humirac_mean = F_humirac(humsol,hn,hn,P_sensrsec)

! ** Ajout d'une contrainte � la p�n�tration  racinaire: efda
! -------------------------------------------------------
...
! dans le cadre du projet PERMED
! #### SYL
! NB le 22/11/07 extension de l'arr�t de pousse des racines quand pas de
! croissance
        if (P_codedyntalle == 1) then
...
! **NB - le 07/06 - stress joue apr�s calcul de znonli
...
! ** entre la floraison et la r�colte, la profondeur du syst�me racinaire n'�volue pas
! *- apr�s la r�colte, de nouveau situation de sol nu
! *- domi - 26/04/2002 - rajout d'un test pour qu'on remette zrac � 0 apres la recolte
...
! --       if (nrec > 0 .and. P_codesimul == 'culture') then
      ...
      if (nrec > 0 .and. n /= nrec .and. trim(P_codesimul) == 'culture' .and. P_codcueille == 1) then
        ...
        cumlracz = 0.
        poussracmoy = 0.
        lracsenz(:) = 0. ! on met � z�ro tout le tableau plutot que lracsenz(n) = 0. qui n'a pas de sens
! *- 17/06/04 - emma
        dtj(n) = 0.
        return
      endif
...
!: Variables locales
  ...
  real    :: S
      ...
      ! les3 - 10/06/03 - nouveau test  pour le demarrage racinaire
        ...
        ! profil de densit� racinaire efficace : tableau lracz((200)
          cumlracz = 0.
        ! Nb - le 05/06 - cumflrac = cumul de racines / cumlracz =  cumul de racines fonctionnelles
          cumflrac = 0.
          zdemi = max((znonli - difrac),(log(4.)/S))
          racnoy = 0.
        ! domi - 14/12/00 -  sol 200 � 1000
        ! PB - 2009 - param�tre nbCouches � la place de la limite en dur
          humirac_mean = 0.
          do iz = 1, nbCouches
          ...
          ! NB - le 08/05/02 - P_sensrsec pour effet s�cheresse
