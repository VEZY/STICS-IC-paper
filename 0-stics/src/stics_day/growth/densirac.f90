...
! * P_draclong : param�tre sp�cifique donnant la vitesse de croissance      * c
! * racinaire par plante et par degr�.jour                                * c
! * drliz : r�partition de la longueur dans chaque couche                 * c
! * poussrac(z) : indice entre 0 et 1 d�finissant les obstacles physiques * c
! * � la croissance                                                       * c
! * P_croirac : croissance du front racinaire en cm par degr�.jour          * c
! * debsen : jour d�finissant l'entr� en s�nescence                       * c
! * ratio de s�nescence                                                   * c
! * prop = 1 r�partition proportionnelle aux racines pr�sentes            * c
! * prop = 0 r�partition �quitable                                        * c
! *                                                                       * c
...
subroutine densiteVraieRacinaire(n,nbCouches,dacouche,hur,humin,anox,nstoprac,P_codazorac,                &
                                 ...
                                 masec,P_codemortalracine,dltmsrac_plante,densiteequiv)
...
USE Messages
  ...
  real,    intent(IN)    :: dtj(n)                      ! 1 � n    // OUTPUT // Daily efficient temperature for the root growing  // degree C.j-1
  ...
  real,    intent(IN)    :: densiteequiv  ! equivalent density compared to a sole crop
  real,    intent(IN)    :: P_msresiduel  ! // PARAMETER // Residual dry matter after a cut // t ha-1 // PARTEC // 1

  real,    intent(IN)    :: lai_veille                  ! lai(n-1), but for both species if intercropping (to consider the competition)
...
!write(245,*)'densirac profsem',drlsenmortalle,P_profsem,densite
      ...
      !: NB & PB - le 25/05/2004: On continue la croissance racinaire apr�s la r�colte
      !-                          si autoris� par nstoprac.
      ...
      do iz = 1, int(zrac)+1
        ...
        !: 1. Effet Da (contrainte � la p�n�tration racinaire) : efda
        !------------------------------------------------------------
        !- Fonction Jones et al. 1991  + th�se de B. Rebi�re, adapt�e avec la densit� apparente
        !- P_daseuilbas et P_daseuilhaut : seuils min et max de densit� apparente
        !- P_contrdamax : taux maximal de reduction de vitesse de croissance racinaire
        ! TODO : que faire qd zrac = 0 ? j'ai mis un bornage pour �viter les d�bordements de tableaux
        daz = dacouche(max(1,int(zrac)))
        efda = ESCALIN(daz,P_daseuilbas,P_daseuilhaut,1.,P_contrdamax)
        if (daz < P_dacohes) efda = daz / P_dacohes

        !: 2. Effet H2O (sensibilit� � l'anoxie) : P_sensanox
        !--------------------------------------------------
        !- NB - le 18/12/01: efpfz remplac� par humirac
        ...
        !: 3. Effet N sur la r�partition des racines (option non activ�e) : efnrac
        !-------------------------------------------------------------------------
      ...
      !: Nb - le 13/02/2003:
      !- Calcul d'une pond�ration incluant pr�sence de racine et contraintes sol

      do iz = 1, int(zrac)+1
        !: par d�faut pondetot(iz) = 1.
        pondetot(iz) = 1.
        !: PB - 24/12/2004: Attention, si poussracmoy = 0, division par z�ro !
...
! TODO: gestion de l'�change entre plante et sol nu...
...
!!!            plante_ori = .FALSE.
      ...
      !: Pond�ration de poussrac pour respecter un cumul de 1
      !- pour l'ensemble du profil
      poussracmoy = max(1.,poussracmoy)


      !: NB - 19/02/2008: mort d'une partie des racines lors de la fauche des cultures fourrag�res
      !- P_coefracoupe permet de pond�rer la mortalit� des racines par esp�ce (entre 0 et 1)
      !- Dr - 22/02/08: pass� en parametre dans paramv6.par
      !--P_coefracoupe = 0.5
      if (sioncoupe) then
        !--durvieracine = 0.0
        !--durvieracine = P_debsenrac*P_msresiduel(numcoupe)/masectot*P_coefracoupe
        !: NB - 13/03/08: On met une courbe � la place de la droite P_coefracoupe est la courbure .
        !- mettre � 0.1 pour le moment
        ! DR 19/02/215 pour FR on test en faisant jouer masec au lieu de masectot pour etre moins severe sur la proportion de
        ! racines � faire mourir � la coupe
        ! voir ce qu'on garde et enlever l'autre
        ! DR 06/05/2015 j'ajoute n code pour tester masec ou masectot pour la mort des racines � la coupe
      ...
      ! DR et ml 24/07/2013 On reactive sinon ce n'est plus une moyenne et il n'est pas utilis� apres
      ...
      !- d�finition de sommes de degr�s.jours racines
      somtemprac = somtemprac + dtj(n)
      if (n == nlev) somtemprac = 0.
      if (nstoprac == 0) then
        !: D�finition d'une unit� de dl racinaire urac
        !- somme de degr�.jours racine
        urac = min(1. + (2. * somtemprac / (P_stlevamf + P_stamflax)), 3.)
        if (nlev == 0) urac = 1.
        !: la longueur totale de racine �mise par jour (rlj)
        !- suit une logistique de fa�on similaire au LAI
        !- NB - le 28/10/01:
        ...
        !--nouvrac = P_lvfront*multlvfront
         nouvrac = P_lvfront*(densite/densiteequiv)*deltaz
        ! RV: add *(densite/densiteequiv) because P_lvfront is given for sole crop, and should be weightd by the density of the plant
        ! for intercrops (densite/densiteequiv = 1 for sole crops)

        !: Introduction de l'indice de stress de densit� racinaire idzrac / NB - le 06/06
        efanoxd = 1.-(1.-idzrac)*P_sensanox

        !: NB - 10/03/02:
        !- Calcul du facteur densit� efdensite_rac actif � partir de P_laicomp
        !- Calcul de l'effet densit� sur la longueur racinaire
        ! DR  et ML 20/04/2016 on rajoute une variable specifique pour les racines car on a deja un efdensite pour les feuilles
        !               qui peut �tre calculee en fonstion de la densite equivalente dans le cas des cultures associees
        efdensite_rac = 1.
        if (urac > 1.) then
          if (lai_veille < P_laicomp) then
            ...
            if (densite == 0) then
              ...
              efdensite_rac = min((densiteequiv / P_bdens)**P_adens, 1.)
            endif
          endif
        else
            if (densite == 0) then
        ...
        !: De la germination � la lev�e, il n'y a qu'une croissance du front racinaire
        !- Nb - le 17/02/2003: suppression de coderac
        if (P_codtrophrac /= 3) then
          !: Option trophique
          ! *- par calcul d'une fonction de r�partition reprac = souterrain/total
          ! *- la longueur de racine au niveau du front est soustraite de la
          ! *- longueur produite
          ! ** ML le 25/05/2007 - les param�tres initiaux repracmin, repracmax et kreprac
          ! *- ont chang� de noms dans le fichier plante: P_repracpermin, P_repracpermax et P_krepracperm
...
!write(1236,*)'1',n,rlj1,nouvrac
      ...
      !: Changement d'unit� de rlj en cm racine/cm2 sol/jour
      rlj = rlj * 1.e-4
     ! write(1236,*)'2',n,rlj,nouvrac
      ...
      !: Jour du d�but de s�n�scence racinaire
      ...
      !: Hypoth�se de longueur sp�cifique de 18e3 cmg-1 pour le bl�
      !- et 9e3 cmg-1 pour le ma�s (Gregory et al., 1997)
      ...
      !: D�but de la boucle profondeur
      !--do 60 iz = int(P_profsem),int(zrac)+1
      !- nb &sb - 26/03/07 : ATTENTION, l'indice de fin de boucle a �t� modifi�,
      !- cela est susceptible de g�n�rer des effets de bords non d�sir�s et +/- g�nants !
      ! A VALIDER !
      tot=0
      do iz = int(P_profsem), int(zrac)

        !: Si on est apr�s le stade nstoprac alors drliz = 0.
        if (nstoprac > 0) then
          drliz = 0.
        else
          ...
          !- (option prop = 0)
          if (prop == 0) then
            !: NB - 13/02/2003 : R�partition �quitable sans tenir compte des contraintes sol
            ...
            !                    rapport � l'�paisseur de la couche, pour rendre ce calcul continu.
            drliz = rlj * epcouche(iz,P_profsem,zrac) / (zrac - P_profsem + 1)
          endif

          !: Distribution de la longueur racinaire au prorata de poussrac
          !- et des racines pr�sentes (option prop = 1)
          if (prop == 1) then
            !: Calcul de la proportion de racines pr�sentes
            if (n > nger.or.codeinstal == 1) then
              if (precrac(iz) <= 0 .and. iz > (zrac - deltaz)) then
                ...
                if (deltaz > 0.) then
                  ...
                  ! dr 24/07/2013 j'affecte � la dernier couche (celle de zrac) le reliquat de nouvrac moins la somme des couches d'avant (tot) pour que le bilan soit bon
        ...
        !: Calcul de la s�nescence racinaire : P_debsenrac * racines �mises  au jour j-debsen
      ...
      ! ####
      ! SYL 120907 Mortalit� d'une proportion de racine en lien avec la mortalit� de talles
...
!          write(1236,*)n,iz,'rlveille',rl_veille(iz), 'drliz',drliz ,'drlsen',drlsen,'rl',rl(iz)

        !: sb - 06/03/2007 :
        !- Lorsque la quantit� de racine devient tr�s faible, c'est que les racines ont disparu.
        !- En pratique, dans ce cas, la longueur de racine ne devient jamais exactement �gale � 0
        !- en raison de la fa�on de calculer la senescence et des erreurs d'arrondis. Cette longueur est
        !- mise � 0 seulement dans les cas o� le calcul la donne n�gative. Pour �viter les impr�cisions
        !- et instabilit� num�riques que cela g�n�re, on a choisi de fixer la longueur de racine � 0 d�s
        !- qu'elle est inf�rieure � une quantit� tr�s faible.
        !--if (rl(n,iz) < 0.) then
...
!     write(1236,*)n,' fin rlj',rlj,'rltot',rltot
      ...
      !- msrac comprend � la fois la mati�re racinaire vivante et morte
      ...
      ! Dans le code et dans le bouquin on annoce rltot (et donc rlj) en cm racine /cm� sol, alors que dans le code on calcule d'autres unit�s. Fort heureusement on retombe sur nos pattes pour msrac qui est bien en t/ha (mais gr�ce � la compensation de deux erreurs).
      ! Il faudrait faire v�rifier cela par d'autres r�f�rents.
