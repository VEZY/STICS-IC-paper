...
real, intent(IN)    :: lai2   ! // OUTPUT // Leaf area index of the other plant if any  // m2 leafs  m-2 soil
...
subroutine senescen(nlev, n, nbjmax, lai, P_codeinnact, P_codeh2oact, senfac, innsenes, P_codlainet, &
    ...
    real, intent(IN)    :: ulai(nbjmax)  ! dans l'id�al max(n,dernier_n) ! (dernier_n)    // OUTPUT // Daily relative development unit for LAI // 0-3
    ...
    real, intent(IN)    :: deltai_sencour(nbjmax)
    real, intent(IN)    :: dltams_sencour(nbjmax) ! Same here
    real, intent(IN)    :: pfeuilverte_sencour(nbjmax) ! Same here
    real, intent(IN)    :: P_lai0  ! // PARAMETER // Initial leaf area index // m2 m-2 // INIT // 1

    ! dltams dans l'id�al max(n,dernier_n) ! (dernier_n) ! ndebsen � dernier_n, puis si n < ndebsen, 1 � n
    ...
    real, intent(INOUT) :: durvie(nbjmax)  ! dans l'id�al max(n,dernier_n) ! (dernier_n) ! ndebsen � dernier_n, puis si n < ndebsen, 1 � n    // OUTPUT // Actual life span of the leaf surface //  degree C
...
! *---------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    real, intent(INOUT) :: Cres(nbCouchesSol, nbResidus) ! premi�re couche, itrav1 = 1 & itrav2 = 1
    real, intent(INOUT) :: Nres(nbCouchesSol, nbResidus) ! premi�re couche, itrav1 = 1 & itrav2 = 1
...
! ajout Bruno; param�tres de d�composition du residu ires (ici 'feuilles mortes')
    real, intent(IN) ::awb
    real, intent(IN) ::bwb
    real, intent(IN) ::cwb
...
! Variables locales
    ...
    real :: Wr           !ajout calcul des param�tres de d�composition du r�sidu "feuilles mortes"
    real :: Nplantetombe
    real :: eaures
...
subroutine grain(n, ndrp, nrec, nlev, nrecbutoir, P_nbjgrain, dltams, P_cgrain, P_cgrainv0, & ! IN
    ...
    real, intent(IN)    :: dltams(ndrp) ! ndrp-P_nbjgrain+1 � ndrp    // OUTPUT // Growth rate of the plant  // t ha-1.j-1
    ...
    real, intent(IN)    :: P_tminremp  ! // PARAMETER // Minimal temperature for grain filling // degree C // PARPLT // 1
    real, intent(IN)    :: P_tmaxremp  ! // PARAMETER // maximal temperature for grain filling // degree C // PARPLT // 1
    real, intent(IN)    :: P_pgrainmaxi  ! // PARAMETER // Maximum weight of one grain (� 0% water content) // g // PARPLT // 1
...
subroutine croissanceFrontRacinaire( &
    ...
    character(len=*), intent(IN)    :: P_codesimul  ! // PARAMETER // simulation code (culture ou feuille=lai forc�) // SD // P_USM/USMXML // 0
...
subroutine densiteRacinaire( &
    ...
    P_codemortalracine, dltmsrac_plante, densiteequiv)
