...
! * - derni�re modif le 22/5/98  param�trage de P_irmax    * c
...
!: Variables locales
  ...
  real    :: tefficace
      ...
      !: magrain reste constant apr�s nrec si P_codemsfinal = 1 (pour optimisation)
!      if (nrec > 0 .and. n > nrec .and. p%P_codemsfinal == 1) then
      ...
      !: Attention incoh�rence entre tsol qui interdit la lev�e et tair qui am�ne � drp
      ...
      !: Modif NB - 20/08/97
      !- Si la date de r�colte butoir intervient avant le d�but remplissage,
      !- pas de grain.
      ...
      if (n == ndrp) then
        ...
        !- NB - 08/04 : nbgrains remplac� par nbgrainpot
        vitmoy = 0.0
        ng = ndrp - P_nbjgrain + 1
        vitmoy = SUM(dltams(ng:ndrp))
        vitmoy = vitmoy / P_nbjgrain * 100

        !: nbgrains devient vari�tal avec P_nbgrmax
        ...
        !: domi - 29/10/05: irazo deplac� dans Ngrain sinon on a pas la valeur
        !-                  de qnplante qui est calcul�e dans stressn
        ...
        if (nmat == 0 .or. n == nmat) then
          ...
          !- NB - le 23/05 : suppression des grains gel�s
          !- NB - passage par dltags pour echaudage actif - le 11/01/02
          !-- magrain(1) = ircarb(1)*masec(n)*100 - pgraingel

          !- DR et AIG, on a trouv� un bug :
          !- On enlevait chaque jour le cumul du poids de grain gel�
          !- au lieu du poids de grain gel� du jour et ce meme apres
          ...
          ! les calculs d'azote dans les grains ont ete deplac� dans Ngrain
        endif

        !: Entre maturit� et r�colte
        !- DR - 050506 : Dans le cas o� on repousse la recolte,
