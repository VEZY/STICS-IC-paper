...
USE Parametres_Generaux
  ...
  type(Plante_),              intent(INOUT) :: p(sc%P_nbplantes)    ! Toutes les plantes du syst�me, pour somme des LAI et des ABSO
...
!  integer :: kk
  ...
  AO = sc%AO
      ...
      !- puis on calcule une valeur de Lai global � la culture (i.e : pour toutes les plantes)
    !TODO: d�placer le calcul de laiTot qui n'est pas utilis� par fertil, le mettre l� o� il est utilis�.
      sc%laiTot = 0.
      ...
      if (itk%P_codedateappn == 1 .and. p(1)%nplt > 0) then
        call calapNenUpvt(sc%n,p(1)%nplt,itk%P_upvttapN(1:300),itk%P_codefracappN,itk%P_doseN(1:300), &
          ...
          ! DR 01/02/2017 il faut aussi prendre en compte la possibilit� de pouvoir faire des ferti de type differents en somme de temp
          if(sc%anit(sc%n).gt.0)then
          !    write(*,*)'apport N'
...
! - (uniquement pour le mode de simulation monoculture donc ipl = 1 et ens = AS)
      ...
      if (t%P_codecalferti == 1) then
        itk%P_engrais = 8 ! P_engrais type 8 : donn�es de pertes forc�es (cf. doc)

        ! le jour du semis, on initialise � z�ro le nombre d'apport en N
        ! TODO: extrait du calcul automatique, � voir o� le mettre
...
! --------------------------------------------------------------
        ! TODO : P_locirrig peut �tre �gal � z�ro, l'indice z�ro de nit n'existe pas, bug ?!
...
! ----------------------------------------------------------
            if(sc%anit(sc%n).gt.0)then
!                write(115,*)sc%numcult,'avant apport  ',p(1)%numcoupe,sc%n,'anit_tot   ', sc%anit(sc%n), &
!                & soil%nit(1),soil%amm(1)
            endif

  !    if(sc%flag_onacoupe .and. itk%P_restit(p(1)%numcoupe).eq.1)then  ! 14/04/2016 fertilisation par les pissats
      if(itk%P_restit(p(1)%numcoupe).eq.1.and.sc%anit_uree(sc%n).gt.0.0)then
            ...
            call apportsNparEngraisMineraux(pg%P_orgeng(t%P_engrais_pature),pg%P_voleng(t%P_engrais_pature), &
                                      ...
                                      soil%QNdenit,sc%Nhum(1:10),sc%Nhuma,soil%amm(1:max(1,itk%P_locferti)), &
                                      soil%nit(1:max(1,itk%P_locferti)), sc%precipN,sc%totapN)
            ! DR 06/04/2016 on remet le flag � faux
            sc%flag_onacoupe=.FALSE.
            if(sc%anit_uree(sc%n).gt.0)then
!                write(115,*)sc%numcult,'apport uree   ',p(1)%numcoupe,sc%n,t%P_engrais_pature, sc%anit_uree(sc%n),&
!                & pg%P_engamm(t%P_engrais_pature),soil%nit(1), soil%amm(1), soil%Nvoleng,soil%Ndenit,soil%Norgeng,&
!                & sc%absoTot
            endif
! DR 07/12/2016 je teste l'application des apports ur�e et feri le meme jour
! je mets un endif � la place du else
      endif
     ! else
! 14/04/2016 fertilisation classique
            call apportsNparEngraisMineraux(pg%P_orgeng(sc%type_ferti(sc%n)),pg%P_voleng(sc%type_ferti(sc%n)),  &
                                      pg%P_deneng(sc%type_ferti(sc%n)), &
                                    !  sc%anit(sc%n),sc%n,sc%absoTot,pg%P_Vabs2,itk%P_codlocferti,           & ! IN
                                      sc%anit_engrais(sc%n),sc%n,sc%absoTot,pg%P_Vabs2,itk%P_codlocferti,           & ! IN
                                      itk%P_locferti,pg%P_pHminvol,pg%P_pHmaxvol,soil%P_pH,pg%P_Xorgmax,    &
                       !               soil%P_codedenit,pg%P_Wh,pg%P_engamm(itk%P_engrais),                  &
                                      soil%P_codedenit,pg%P_engamm(sc%type_ferti(sc%n)),                  &
                                      soil%Nvoleng,soil%Ndenit,soil%Norgeng,soil%QNvoleng,soil%QNorgeng,   & ! INOUT
                                      soil%QNdenit,sc%Nhum(1:10),sc%Nhuma,soil%amm(1:max(1,itk%P_locferti)), &
                                      soil%nit(1:max(1,itk%P_locferti)), sc%precipN,sc%totapN)
            if( sc%anit_engrais(sc%n).gt.0.0)then
!                write(115,*)sc%numcult,'apport engrais',p(1)%numcoupe,sc%n,sc%type_ferti(sc%n), sc%anit_engrais(sc%n),&
!                & pg%P_engamm(sc%type_ferti(sc%n)),soil%nit(1), soil%amm(1), soil%Nvoleng,soil%Ndenit,soil%Norgeng,&
!                & sc%absoTot
            endif
! DR 07/12/2016 je teste l'application des apports ur�e et feri le meme jour
! je supprime le endif
!     endif

! *---------------------------------------------------------------
! 4. Apport d'amendements organiques  et/ou travail du sol
! *-    Cons�quences: - apport d'azote min�ral sous forme NH4
! *-                  - apport de diff�rentes formes de MO
! *-                  - m�lange de ces MO par le travail du sol
! *-  P_profres  = cote sup�rieure d'incorporation des r�sidus
! *-  P_proftrav = profondeur de travail du sol = cote inf�rieure
! *---------------------------------------------------------------

      ! TODO : que se passe-t-il si napS = z�ro ??
...
! DR 26/07/2011 pb sur l'appel de fonction mauvaise prise en compte de numjtrav
      ...
      ! Si on simule + d'1 plante, alors on recalcule un precipN (� pr�ciser)
