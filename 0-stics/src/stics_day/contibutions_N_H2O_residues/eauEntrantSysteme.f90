...
! DR 16/11/2015 je gere le demarrage des irrigations auto dates ou stades ici
   ...
   call start_irrig_auto(n,p(1)%nplt,p(1)%nger,p(1)%nlev,p(1)%namf,p(1)%nlax,p(1)%ndrp,p(1)%nflo,p(1)%nsen,&
                     &  p(1)%nrec,p(1)%nmat,p(1)%ndebdorm,p(1)%nfindorm,  &
                     &  t%P_stage_start_irrigauto,t%P_stage_end_irrigauto,sc%n_datedeb_irrigauto,sc%n_datefin_irrigauto)
   endif
