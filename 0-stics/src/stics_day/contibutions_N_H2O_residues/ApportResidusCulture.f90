...
subroutine ApportResidusCulture(n,nrec,cumvminr,P_codcueille,ntaille,P_coderacine,masec,P_proprac,P_y0msrac,msrac_prec,  & ! IN
                               ...
                               CsurNressuite_tot,QCrac,QNrac,QNplantefauche,P_codeplante,matuber)
...
! DR 19/09/2016 on fait en sorte de calucler des residus correct pour la betterave
...
! DR 19/09/2016 pour la betterave ce n'est pas magrain qu'il faut enlever mais le matuber
        if (index(P_codeplante , 'bet')>0 ) then
             mspaille = masec - matuber
        endif
