...
!
! 1) Calcul d'une fixation potentielle fixpot (kg/ha/j), r�sultant de
!     la mise en place des nodules entre les stades d�but nodulation (DNO)
!     et fin nodulation (FNO), faisant intervenir  :
!   * une vitesse d'installation P_vitno (ratio de P_fixmax degr�.jour-1)
!      P_fixmax = 6 kg/ha/j (Bouniols et al., 1991)
!   * une dur�e de vie de la population DVIENO (degr�s.jours)
!   *  un effet inhibiteur du N min�ral � partir du seuil de concentration
!      NNODMAX dans la zone de nodulation (P_profsem � P_profnod)
...
!   * contrainte azote min�ral (FXN) faisant intervenir la variable
!      CUMOFFRN et les seuils FXN0 et FXN100
!   * contrainte s�cheresse (FXW) calcul� comme la proportion des
!      couches de sol ayant une humidit� sup�rieure au seuil HURNOD
!      entre P_profsem et P_profnod
!   * contrainte thermique (FXT) faisant intervenir 4 temp�ratures
!     cardinales P_TEMPNOD1, P_TEMPNOD2, P_TEMPNOD3 et P_TEMPNOD4 (Tmoyenne
!     ou maxi) calcul�es au niveau de chaque couche
...
!: Arguments
  ...
  real,    intent(IN)    :: P_vitno  ! // PARAMETER // "rate of nodule onset expressed as a proportion of  P_fixmax  per degree day " // nb degr�s.jour-1 // PARPLT // 1
  ...
  real,    intent(INOUT) :: P_stlevdno  ! // PARAMETER // phasic duration between emergence and the beginning of nodulation  // degr�s.jours // PARPLT // 1
  ...
  real,    intent(INOUT) :: P_stfnofvino  ! // PARAMETER // phasic duration between the end of the nodulation and the end of the nodule life   // degr�s.jours // PARPLT // 1
...
!     codecombi = 1
      ...
      ! --------------------------------------!
      !  1. Calcul des sommes de temp�rature  !
      ! --------------------------------------!
...
!        write(ficdbg,*) 'fvino',n,somcourno,P_stfnofvino,dtj
      ...
      ! ----------------------------------------------------!
      ! 2. Calcul de la contrainte azot�e sur fixpot: nodn  !
      ! ----------------------------------------------------!
      azonod = 0.0
      hurnod = 0.0
      ...
      ! ---------------------------------------------!
      !- 30/04/04 - fixpot entre 0 et 1 (P_fixmax remplac� par 1.0)
      !- 10/06/2004 - pb avec l'initialisation de fixpotfno - il faut le mettre a z�ro !
      !- on introdiut une variable intermediaire propfixpot sinon pb apres nfno
      if (n < ndno .and. ndno > 0) then
        propfixpot = 0.0
        ! print *, "propfixpot from 1"
      endif
      if (n > nfvino .and. nfvino > 0)then
        propfixpot = 0.0
        ! print *, "propfixpot from 2"
      endif
      if (n >= ndno .and. ndno > 0 .and. nfno == 0) then
        propfixpot = propfixpot + (1.0 * P_vitno * dtj * nodn)
        propfixpot = min(propfixpot, 1.0)
        ! print *, "propfixpot from 3"
...
!      write(ficdbg,'(i4,a15,f16.12)')n,'fixpotfno = ',fixpotfno
      ...
      !: Option 1 : P_fixmax fix�
      if (P_codefixpot == 1) then
        fixmaxvar = P_fixmax
        fixpot = propfixpot * fixmaxvar
      endif

      !: Option 2 : P_fixmax pilot� par la croissance
      !- ML le 25/05/07: l'option 2 (en fonction de la demande en azote)
      !                  a �t� supprim�e: l'option 3 devient donc l'option 2
...
!   de fixation symbiotique
      ...
      fxw = 0.0
      do iz = int(P_profsem),int(P_profnod)
        ! sb le 05/03/07 : on essaie de rendre �a un peu plus continue ...
...
! --      endif
      ...
      ! --------------------------------------------------------------- !
      !  8. Calcul de l'effet combin� temp�rature, eau, azote et anoxie !
      ! --------------------------------------------------------------- !
      !    Choix 3 P_Ph. Burger
