! ****************************************************** c
! * version 5.1 18/03/98 - derni�re modif : 03/02/2004 * c
...
!: Variables locales
  ...
  ! domi 02/10/00 si on est en fauche P_codefauche = 1
    ...
    ! 05/05/2015 DR et FR on ajoute la variable mafruit r�colt�
    ...
    ! On d�termine si on est un jour de coupe,
    ! si oui, on met sioncoupe � true
    ! si non, on met sioncoupe � false

      ! DR 09/06/2016 on essaie d'eviter les coupes de fin d'annee quand on a depass� le nb de coupes du tec
...
! DR et FR 12/07/2016 on n'est jamais en fauche repouss�e uen debut d'ann�e
...
!                  .and. (sc%nbcoupe_reel(p(i)%ipl)<=itk(i)%nbcoupe) )               &
      ...
      ! param�trage du stade de coupe automatique NB le 20/08/97
      ...
      ! on teste les criteres de coupe
      ! ce qui revient � dire qu'on coupe si les 2 sont respectees ...
...
!          if (p(i)%mafruit < itk(i)%P_mscoupemini .or. p(i)%lai(0,sc%n) < itk(i)%P_lairesiduel(p(i)%numcoupe)) then
       ...
       ! dr et fr 16/02/2016le teste est mauvais on ne recolte que si on a au moins mscoupe_mini � recolter
...
!            return
    ...
    ! �criture de recup.tmp et calcul de msrac dans recup.for
    ! PB - 07/12/2004 - d�plac� de bilcoupe pour l'enchainement des p�rennes.
...
!        else
        if (sc%n == sc%maxwth) then
          ...
          ! copi� et coll� ici pour correspondre au comportement avant modification pour modularisation
...
! faudrait qu'on le fasse dans tous les cas a la fin de l'ann�e meme si on est pas en fauche reposs�e, cas ou on a pas atteint la somme
! ce cas la n'arrive que quand on est en somme
! TODO

            p(i)%ulai0 = p(i)%ulai(sc%aoas,sc%n)
...
! 15/04/2016
        ...
        ! 27/12/2016 on doit aussi ecrire la fin de bilan quand on a plus de coupes que prevues, c'est qu'on laisse pousser � la fin
         if(p(i)%fauchediff) then
        !if(p(i)%fauchediff.or.(p(i)%numcoupe.gt.itk(i)%nbcoupe)) then
          ! DR 29/11/2016
          ! le jour de la r�colte butoir
               if (sc%n == p(i)%nrecbutoir) then
                    if (iand(pg%P_flagEcriture,sc%ECRITURE_BILAN) > 0) then
                          call ecrireBilanFinCultureFauchee(sc,pg,soil,p(i),itk(i))
 ! E et B 02/08/2012 on change le nom de la routine qui sera commune � toutes les cultures
...
! TODO
    ...
    ! le jour de la r�colte butoir
    ! 27/12/2016 ce test ne suffisait pas dans le cas ou nrecbutoir n'etait pas renseign�
    !  if (sc%n == p(i)%nrecbutoir) then
	...
	! 20/08/2012 DR et j'ai modifi� ecrirebilanrecoltebutoir pour n'avoir que le bilan de BM
...
! variables locales
  integer :: kcoupe
    ...
    !---------------------------
    ! modif domi - 13/11/97 - � la premiere coupe on stocke les valeurs de sommes de Temp
    ! pour repartir � un stade de developpement de croissance des feuilles, ulai equivalent
      ...
      if (numcoupe == 1) then
        do kcoupe = 1,nbcoupe
          if (udevlaires(kcoupe) == 0 .and. lai >= P_lairesiduel(kcoupe)) then
            ...
            !write(*,*) numcoupe, kcoupe,P_lairesiduel(kcoupe), somcour
          endif
        end do
      endif

! *- pour l'instant lai = lai+P_lairesiduel
! *- PB - 06/01/2005 - mise en commentaire dans le cadre de l'enchainement des p�rennes
