...
!
! indice P = relatif � la culture principale
! indice A = relatif � la culture associ�e
! entree clim : vent(ms-1),tempe(degree C),rayonnement net(MJm-2j-1),
! pression de vapeur (mbars)
! test par rapport proc�dure Matlab (beaucas.m) le 16/3/98 OK
...
subroutine shutwall(sc, pg, c, sta, soil, p, itk, t) ! DR 19/07/2012 t n'est pas utilis�
...
! domi 12/01/06 certaines varaibles ne sont plus utilis�es je les met en com en attendant
!  real   :: largeurP,fracinsol,Rsolglo,rnetpen,RGEX,largeurA

! On fixe la dimension � 2 mais dans l'absolu il faudrait dimension(P_nbplantes) : ou pas ? on peut aussi dire que la routine ne marche que pour 1 � 2 plantes.
   real    :: hauteurMoy(2)
   real    :: termrad(2)
   real    :: termres(2)
   real    :: rac(2)
...
!: Fonction(s)
   ...
   mouillA = 0.0
   ! DR 28/04/2011 c'etait desactiv� , je le reactive sinon pb
   rnetA = 0.0
   rnetP = 0.0
   ...
   ! Constantes pour le calcul des r�sistances
   rb = 50
   nconv = 2.5
   ...
   ! d�termination du sch�ma r�sistif "haut" ou "bas"
   if (deuxcult) then
...
12 continue
   ...
   ! TODO : laiTot d�j� calcul� auparavant dans sc%laiTot
   sc%laiTot = 0.
   do ii = 1, sc%P_nbplantes
...
! DR et ML 09/02/2011 on fait 2 appel suivant le type de mulch
   if (itk(1)%P_codepaillage == 2) then   !! paillage mulch plastique
      call calrnet(sc%nbCouchesSol, sc%jul, sc%hur, sc%humin, sc%hucc, sc%laiTot, sc%tauxcouv(n), sc%posibsw, &   ! IN
                   ...
                   c%tpm(n), sta%P_codecaltemp, sc%albedolai, sc%rglo, sc%rnet)                                   ! INOUT
   else
...
!#endif
   ...
   ! �nergies disponibles au niveau des trois strates (Thornley, 1996)
   ! r�partition des �nergies entre AO et AS
   if (c%trg(n) <= 0.0) then
   ...
   f1 = p(1)%raint(AS)/pg%P_parsurrg/c%trg(n)
   rnetP = t%P_par_to_net*f1*sc%rnet
...
!        write(ficdbg,*)'1.1',n,p(2)%raint(AO) , p(2)%surf(AO) , p(2)%raint(AS) , p(2)%surf(AS)
      f2 = raintA/pg%P_parsurrg/c%trg(n)
      rnetA = t%P_par_to_net*f2*sc%rnet
...
!      write(ficdbg,*)'2.',n,sc%rnet,rnetA,rnetP
   ...
   ! le mulch joue le r�le d'une culture associ�e
...
!      coefficient d'�change sol nu sous la culture, sc%ras et de raa
!      seuillages
   if (sc%P_nbplantes > 1) then
      if (hauteurMoy(2) > hauteurMoy(1)) then
         call EnvoyerMsgHistorique(161, sc%n)
!          stop
      end if
   end if
...
! DR_2010        write(*,*)'2',hauteurMoy(2),deuxcult,p(2)%lai(0,n-1)
      ! on d�coupe le test pour ne pas tester p(2) si p(2) n'existe pas.
...
!      ,soil,hauteurmoy
   ...
   ! pour le calcul de la volatilisation et des temp�ratures de culture
...
! r�sistances de surface
! ** le sol

   ! la culture principale
   if (p(1)%lai(AS, n) > 0.0) then
      p(1)%rc = p(1)%P_rsmin*(0.5*p(1)%lai(AS, n) + 1)/p(1)%lai(AS, n)
      ! effet dsat et rayonnement  d'apr�s Stockle et Kjelgaard (1996)
   ...
   if (deuxcult) then
      ...
      ! r�sistance a�rodynamique du niveau bas selon le sch�ma r�sistif
      if (coderesist == 2) rac(2) = rac(2) + sc%ras
   end if

   ! calcul des �vaporations directes sol + surfaces feuilles
   ! mouill�es en passant par Priestley-taylor
   ! LE SOL
...
! On est surprise par le faut que tout se passe comme si le rnet et l'evap sol etaient dans la meme unit�
! ligne 406 et 411
...
!Elsa 01/08/2012 attention le call de solnu est incorrect!! des arguments semblent avoir �t� enl�v�s!
...
!                 sc%ses2j0,sc%sesj0,sc%smes02,sc%stoc,sc%nstoc,sc%sum2,sc%esz,sc%esol,sc%esreste,sc%xmlch1,sc%xmlch2)
   ...
   call solnu(sc%n, sc%nbCouchesSol, sc%precip, c%nitetcult(sc%n), soil%P_q0, soil%aevap, &
              soil%P_cfes, pg%P_codeactimulch, sc%ha, sc%hi, sc%hpf, sc%hurlim, soil%P_zesx, sc%hur, sc%hucc, &
              sc%eos, soil%sumes00, soil%sumes10, soil%sumes20, soil%supres0, soil%ses2j00, soil%sesj00, &
              soil%smes020, soil%stoc0, soil%nstoc0, sc%sumes0, sc%sumes1, sc%sumes2, sc%supres, &
              sc%ses2j0, sc%sesj0, sc%smes02, sc%stoc, sc%nstoc, sc%sum2, sc%esz, sc%esol, sc%esreste, sc%xmlch1, sc%xmlch2)
...
! LES FEUILLES
! on estime que l'�vaporation directe sur les feuilles
! consomme une partie du rnet de ces surfaces
   ...
   if (deuxcult) then
      ...
      ! r�partition entre AO et AS en fonction de l'�nergie re�ue
      EmdAO = fAO*p(2)%Emd
      EmdAS = fAS*p(2)%Emd
...
!    if (deuxcult) then
!       ! on d�coupe le test pour ne pas lire p(2) si p(2) n'existe pas.
!       if (p(2)%P_codetransrad == 2) then
!          p(2)%largeur = (hauteurMoy(2) - p(2)%P_hautbase)/abs(p(2)%varrapforme)
!       end if
!    end if

!    if (p(1)%P_codetransrad == 2 .and. p(1)%lai(AS, n) > 0.0) then
!       p(1)%largeur = (hauteurMoy(1) - p(1)%P_hautbase)/abs(p(1)%varrapforme)
!    end if

   ! R�partition entre AO et AS
   p(1)%eop(AS) = eopP
   p(1)%eop(AO) = 0.0
   ...
   ! r�affectation de la r�sistance a�rodynamique pour le calcul de tcult
   sta%P_ra = raa
...
! --      if (nitetcult(n) <= 1) then
      if (c%nitetcult(n) <= 5) then
         c%tutilrnet = sc%tcult
         goto 12
      end if
   end if

   p(1)%rnet_plant = rnetP
   p(2)%rnet_plant = rnetA
! DR_2010 if(n.eq.1)pause
   return
end subroutine shutwall

! d�clinaison    (cf raytrans)
