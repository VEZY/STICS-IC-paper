...
! * version 4.0 selon deux modalit�s possibles                                 * c
! * P_codebeso = 1 : approche k * etm                                            * c
! * P_codebeso = 2 : approche r�sistive (Penman-Monteith,Shuttleworth & Wallace) * c
! ****************************************************************************** c
! ml_com !
! *-----------------------------------------------------------------------------------------------------------------------------------------------------------* c!
...
USE Sol
  ...
  integer :: i
    ...
    !: Calcul de l'�vaporation potentielle
...
!                 esreste,xmlch1,xmlch2)
 ...
 !                  sc%laiTot,sc%tauxcouv(sc%n),c%nitetcult(sc%n),soil%P_q0,soil%aevap, &
        call solnu(sc%n,sc%nbCouchesSol,sc%precip,  &   ! IN
                   c%nitetcult(sc%n),soil%P_q0,soil%aevap, &
...
!#endif
        ...
        call ketp(sc%n,p(1)%P_codelaitr,p(1)%lai(sc%AOAS,sc%n),sc%tauxcouv(sc%n),c%tetp(sc%n),pg%P_beta,    &
                  sc%delta,p(1)%P_codeplante,sta%P_corecTrosee,sc%Emulch,p(1)%P_kmax,t%LAIapex(1),sc%posibsw, &
...
!#endif
      ...
      do i = 1, sc%P_nbplantes
        ...
        call shutwall(sc,pg,c,sta,soil,p,itk,t)  ! DR 19/07/2012 t n'est pas utilis�
