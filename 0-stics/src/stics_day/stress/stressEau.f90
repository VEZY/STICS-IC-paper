...
subroutine stressEau(n,P_profsem,zrac,nbCouches,hur,humin,cumlracz,surf,P_codelaitr,lai,tauxcouv, &  ! IN
                     ...
                     resrac,swfac,turfac,tetstomate,teturg,senfac)                               ! INOUT
...
!: Variables locales
  ...
  integer :: iz
      ...
      ! ** calcul de la r�serve dans la zone racinaire, resrac
      ! *- 17/06/04 - les 3 : donc � partir de P_profsem (bug !)
      resrac = 0.
      ...
      ! ** PB - on pond�re cumlracz par la surface de la plante
      cumlr = cumlracz * surf
...
!      write(618,*)'cumlr', n, cumlr, eop
      ...
      if (     (P_codelaitr == 1 .and. lai <= 0.)          &
        ...
        if (cumlr > 0.) then
!      write(618,*)'cumlr'
                ! ** calcul du seuil de teneur en eau en de�a duquel il y a r�duction de transpiration
          ...
          ! *- changement des param�tres de la relation K(teta) pour �tre conforme avec Taylor et Klepper, 1975
          ! *- K = 10-8exp(80(teta-tetaPF)) et non K = 10-7exp(40(teta-tetaPF))

          tetstomate = 1 / 80. * log(eop / (10. * slrac * 2 * 3.14e-8 * P_psisto * 1e3))

          !: Calcul du seuil de teneur en eau en de�a duquel il y a r�duction d'expansion foliaire
          teturg = 1 / 80. * log(eop / (10. * slrac * 2 * 3.14 * 1e-8 * P_psiturg * 1e3))

          !write(70,'(i4,4f16.12)')n,teturg,slrac,P_psiturg,eop

          ! ** calcul du seuil de teneur en eau en de�a duquel il y a acc�l�ration de s�nescence
          ! ** domi - 22/05/01
