...
! offre = cumul sur le profil de sol de l'offre ï¿½lï¿½mentaire
!         au niveau racinaire estimï¿½e comme le minimum entre
!         l'offre physique (migration du nitrate du sol ï¿½ la
!         racine) et la capacitï¿½ d'absorption (double MM)
!
! fluxrac : flux d'absorption maximal possible par couche  de sol
! fluxsol : flux de transfert sol --> racine   par couche  de sol
! flurac  : flux cumulï¿½ d'absorption sur l'ensemble du profil
! flusol  : flux cumulï¿½ sol --> racine sur l'ensemble du profil
! offrN     = minimum(fluxsol, fluxrac)
! cumoffrN  = cumul de l'offre sur l'ensemble du profil
! masecdil  = matiï¿½re sï¿½che dï¿½terminant les besoins (t/ha)
! P_masecNmax = matiï¿½re sï¿½che ï¿½ laquelle la dilution d'azote commence
! QNplante  = quantitï¿½ d'azote dans la plante (kg/ha)
! NC        = concentration critique en azote
! CNplante  = concentration d'azote dans la plante
! inn       = indice de satisfaction des besoins azotï¿½s
! inns      = inn bornï¿½ ï¿½ 1
!****************************************************************
! ml_com !
! *-----------------------------------------------------------------------------------------------------------------------------------------------------------* c!
...
!!
!! All nitrogen stress indices accept INNmin or INNimin as the floor value for respectively the ï¿½INNï¿½ and the ï¿½INNIï¿½ options.
...
subroutine stressN(masecdil,abso,offrenod,QNplante_prec,dltaremobilN,P_codelegume,P_masec0,P_adilmax, & ! IN
                   P_masecNmax,P_bdilmax,masecpartiel,magrain_abso,P_codeplisoleN,P_masecmeta,adilI, &
                   bdilI,P_adil,P_bdil,adilmaxI,bdilmaxI,dNdWcrit,deltabso,P_codeINN,P_INNmin,P_INNimin,  &
                   P_innturgmin,P_innsen,P_QNpltminINN,                                               &
                   QNplante,Qfix,QNplanteres,CNplante,inn,inni,inns,innlai,innsenes)              ! INOUT

  implicit none

!: Arguments

  real,    intent(IN)    :: masecdil    ! // INPUT // biomass used for the dilution curve (considering picking if any), =masec if sole crop, or masec(1)+masec(2) for intercrops
  ...
  real,    intent(IN)    :: magrain_abso ! total fruit biomass that need N !
...
!: Variables locales
  ...
  real :: dQNc
      ...
      ! print *, "masecdil",masecdil,"masecpartiel",masecpartiel
...
!                   QNplante,Qfix,QNplanteres,CNplante,inn,inni,inns,innlai,innsenes
      ...
      ! *-------------------------------------------------------------------* c
      ! * Calcul des teneurs en azote et des indices de nutrition azotï¿½e * c
      ! *-------------------------------------------------------------------* c
      ! variable VabsN nï¿½cessaire pour INNinstantanï¿½ NB le 07/04/05
      ! domi 16/09/05 affrenod,Qfix passï¿½s en AO/AS
      vabsN = abso + offrenod
      QNplante = QNplante_prec + VabsN

      ! ajout des remobilisations azotï¿½es venant des rï¿½serves
      ...
      ! ** quantitï¿½ minimale d'azote = azote contenu dans les grains
      ! --     CNsemence = 2.0
      ! --     QNplantemin = CNsemence*P_pgrainmaxi*densite*10000/1000/100
      ! --     QNplante = max(QNplante,QNplantemin)

      ! ** pour les cultures plantï¿½es (P_masec0>0) il faut ajouter QNplanteres   NB le 23/4/98
      ...
      masecabso = masecdil - magrain_abso
