! sous-programme qui cumule les variables d'�tat ombre et soleil
! pour ne faire qu'une sortie pour la culture
! TODO: faire des cumuls au fur et � mesure des calculs et non pas tout � la fois en fin de boucle journali�re.
! subroutine that combines the state variables shade and sun
!! to make only outlet for culture
subroutine cumAOetAS(n, P_codesimul, p, itk, par)
...
! *- utilis�s normalement uniquement dans les fonctions de sorties
! *- (et notamment sortie.for et lecsorti.for)
! *- et les variables "xxx(0)" sont les memes variables mais � port�es
! *- plus large que sortie.for et lecsorti.for
! *- typiquement, on les retrouve dans rapport.for et lecrap.for,
! *- ou bilan.for et bilcoupe.for, et parfois ailleurs mais tr�s rarement.
   ...
   p%dltamsen(0) = p%dltamsen(AS) + p%dltamsen(AO)
   ...
   ! Carefull, this is the fapar relative to the PAR coming from the atmosphere, not relative
   ! to the PAR transmitted to the crop. For this one use the fapar computed in croissance.f90
   p%fapar(0) = p%raint(0)/par
...
! *** // variables � cumuler (une seule fois !) //
   ...
   if (lge(P_codesimul, 'feuille') .eqv. .FALSE.) then
      ...
      p%laisen(0, n) = p%laisen(0, n - 1) &
                       + (p%laisen(AO, n) - p%laisen(AO, n - 1)) &
                       + (p%laisen(AS, n) - p%laisen(AS, n - 1))
   ...
   p%ulai(0, n) = p%ulai(AS, n)*p%surf(AS) + p%ulai(AO, n)*p%surf(AO)
...
!: Cumul des rendements
   ...
   !- sauf s'il a d�j� �t� calcul� (par ex. dans inicoupe)
...
!        write(*,*)'dans cumaoas',p%masec(0,n),p%masec(0,n-1) , p%dltams(0,n) , p%dltamstombe(0)
   ...
   ! Si r�colte/cueillette, on enleve la partie r�colt�e/cueill�e du cumul de masec
...
! *- n = nrec
! DR 05/12/2014 attention on a deja enleve une fois la rdt recolt�
!          p%masec(0,n) = p%masec(0,n) - rdtote
   ...
   if (p%masec(0, n) > 0.) then
      ...
      p%pfeuil(0, n) = p%mafeuil(0)/p%masec(0, n)
...
! *!* -- masec(0,n) = masec(0,n-1)+msres0
   ...
   p%potential_height(0) = p%potential_height(AO)*p%surf(AO) + p%potential_height(AS)*p%surf(AS)
   p%largeur(0) = p%largeur(AO)*p%surf(AO) + p%largeur(AS)*p%surf(AS)
   p%varrapforme(0) = p%varrapforme(AO)*p%surf(AO) + p%varrapforme(AS)*p%surf(AS)
...
! Cumul d�j� effectu� apr�s l'appel � la routine "grain"
!      p%nbgrains(0) = p%nbgrains(0)                                         &
!                    + (p%nbgrains(AO) - p%nbgrainsv(AO)) * p%surf(AO)       &
!                    + (p%nbgrains(AS) - p%nbgrainsv(AS)) * p%surf(AS)

   ! PB : 24/07/2009 - Ces variables ao/as ne poss�daient pas de variables de cumuls dans la 6.4
   ! mais dans la version modularis�e, toutes les variables ao/as ont aussi un indice aoas de cumul
   ...
   if (p%masec(AO, n) <= 0.0 .and. p%masec(AS, n) <= 0.0) then
      ...
      !--  msresjaune(0) = 0.0
      p%msres(0) = 0.0
      ! on saute ces mises � z�ros, valables que pour la plante associ�e
      if (.not. p%estDominante) then
         p%msneojaune(0) = 0.0
!DR 27/09/2012 etrange de mettre nbgrains � 0 pour la plante domin�e ?? j'enleve pour nbgrains et nbfruit
...
! ** PB - 06/02/2004
! *- en realit�, lai(0) n'est plus la somme des deltai donc, on a plus besoin de
! *- faire la difference entre le "cumul des deltai" et le "cumul des delta lai senescent"
! *- pour retrouver le lai "r�el" du jour courant.
! -- <obsolete>
! -- on transfere �a l� ou il y en a besoin (lecsorti et sortie)
...
! -- </obsolete>
   ...
   p%abso(0, n) = p%abso(AO, n)*p%surf(AO) + p%abso(AS, n)*p%surf(AS)
...
! RV: no need to do that, it is already computed in Stics_Jour.f90,
! also the computation below uses masec but it should use masecpartiel
! as in the stressN subroutine.

!: Variables recalcul�es

!       if (.not. p%estDominante) then
!         if (p%magrain(0,n) > 0.0) then
!           ! bug corrig� le 13/06/05 NB car magrain est en g/m2
...
!           ! Add CNplante boundaries using P_adilmax
!           if (p%CNplante(0) > p%P_adilmax) then
!             p%CNplante(0) = p%P_adilmax
!           endif
...
!         p%ef_n_w_height(0) = min(p%ef_n_w_height(AO), p%ef_n_w_height(AS))
!       else
...
!         p%ef_n_w_height(0) = p%ef_n_w_height(AS)
!       endif
