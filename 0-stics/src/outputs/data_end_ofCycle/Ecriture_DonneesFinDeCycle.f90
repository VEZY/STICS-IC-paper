...
! pour record
  ...
  ib1 = len_trim(sc%datapath)                             ! enabling_record
  ib2 = len_trim(sc%sticsid)                              ! enabling_record
  if (ib1 .eq. 0 ) then                                   ! enabling_record
     filepluspath = 'recup.tmp'
  else
     if (ib2 .eq. 0 ) then
        filepluspath = sc%datapath(1:ib1) // '/tmp/' // 'recup.tmp'  ! enabling_record
     else
        filepluspath = sc%datapath(1:ib1) // '/tmp/' // 'recup_' // sc%sticsid(1:ib2) // '.tmp'  ! enabling_record
     endif
...
!      open(12,file = 'recup.tmp',status = 'unknown')
      ...
      if (iand(pg%P_flagEcriture,sc%ECRITURE_DEBUG) >0)then
           write(sc%ficdbg,*)'ecriture_recup n',sc%n,(sc%HR(i),i=1,5),(soil%AZnit(i),i=1,5),(sc%AZamm(i),i=1,5)
       endif
...
!      write(12,620) (sc%TS(i),i = 1,5)
      ...
      write(12,*) (sc%HR(i),i = 1,5)
      write(12,*) (soil%AZnit(i),i = 1,5)
      write(12,*) (sc%AZamm(i),i = 1,5)
      write(12,*) (sc%TS(i),i = 1,5)
...
!                    soil%P_mulchbat,soil%P_zesx,soil%P_cfes,soil%P_z0solnu ,soil%P_CsurNsol, soil%P_penterui
      ...
      write(12,*) soil%P_numsol,soil%P_typsol,soil%P_argi,soil%P_Norg,soil%P_profhum,soil%P_calc,soil%P_pH,    &
                    soil%P_concseuil,soil%P_albedo,soil%P_q0,soil%P_ruisolnu,soil%P_obstarac,soil%P_pluiebat, &
                    soil%P_mulchbat,soil%P_zesx,soil%P_cfes,soil%P_z0solnu ,soil%P_CsurNsol, soil%P_penterui
...
703   format(i5,5f10.2,i3,f10.2,i3)
      ...
      if (sc%n >= sc%maxwth) then
        ...
        ! DR 31/05/2017 j'enleve cette ecriture qui fait un message bizarre et non explicite
