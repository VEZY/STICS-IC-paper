...
! ****
!EC et BM 02/08/2012 On pense que cette routine est inactive et pourrait �tre supprim�es (aucun call)
! *********************************************************
...
! E et B 02/08/2012 on change le nom de la routine qui sera commune � toutes les cultures DR 20/08/2012
...
subroutine ecrireEnteteBilan(sc,pg,p,itk,soil,sta,t,usma)  !DR 19/07/2012 c n'est pas utilis�
! *********************************************************
...
! DONNEES d'ENTREES �crites en debut de fichier bilan

    ! DR 20/07/06 on rajoute l'ann�e sinon c'est le bazard
        if (sc%numcult > 1 .or. itk%P_codefauche == 1) &
          write(fb,mes3181) sc%annee(sc%ifwater_courant)

    ! on ins�re une ligne blanche
!        write(fb,'(/)')
...
!   write(*,*)'----',sc%P_wdata1,sc%P_wdata1(1:index(sc%P_wdata1,'.')-1)
...
47        format(5x,a3,7(1x,f6.2))
        ...
        call julien(sc%P_iwater,sc%ansemis,sc%mois,jour,nummois)
...
! FERTILISATIONS
! �criture du bilan des fertilisations
! write(*,*)'ferti final'
    call bilanFertilisations(sc,itk,1,sc%n,fb,'final')

! ** APPORTS DE RESIDUS DE CULTURE
    write(fb,mes1029)
! Travail du sol / Apports de r�sidus organiques
...
! 3) incorporation de residus
     ...
     ! ** Bruno - impression de la quantit� de MS et non de MF pour tester les erreurs d'entr�e
...
! pour que la premiere annee on ne calcule pas de lev�e (debourrement)
! ces 3 conditions servent � sortir du bilan pour ne pas ecrire des stades faux
...
!
    ...
    ! parcours de d�veloppement de la culture
      !write(fb,mes1051)
      write(fb,mes2076)
      ...
      !TODO: d�couper les appels � l'�criture des stades de d�veloppement par paquet
      !       pour intercaler des �critures. Ex : mes1053 = "('    Stades v�g�tatifs')"
      !      Pour diff�rencier les types de stades dans le bilan
...
!  Fin modifs Bruno
    ...
    ! nombre de jours remplissage bloqu� par la temp�rature
    ...
    ! indice de r�colte, sucre et huile
      p%QCplante = 0.
   ...
   ! modif Bruno mai 2012 : calcul de la quantit� de C dans la plante (kg C/ha)
...
! ---------------------------------------------------------------
!   4. Bilan eau, stress hydrique, azot� et GEL sur la culture
! ---------------------------------------------------------------
      write(fb,mes1063)
...
! 24/01/2011 DR pb sur le test des seuils je corrige
    ...
    endif ! fin des r�sultats culture


  ! Ecriture des bilans EAU, AZOTE, CARBONE sur toute la p�riode de simulation
  ! E et B 02/08/2012 on cange le nom de la routine qui sera commune � toutes les cultures DR 20/08/2012
  !  call bilanSimulationDefaut(sc,soil,p,itk) ! DR 19/07/2012 pg non utilis�
  call ecrirebilanEauCN(sc,soil,p,itk) ! DR 19/07/2012 pg non utilis�
...
! DR 20/07/06
! on ecrit si on a repouss� la fauche
...
! IRRIGATIONS
! �criture du bilan d'irrigation pour la coupe
    call bilanIrrigation(sc,p%nplt,sc%n,fb,'coupe')

! FERTILISATIONS
! �criture du bilan des fertilisations pour la coupe
   ! write(*,*)'ferti coupe'
    call bilanFertilisations(sc,itk,p%nplt,sc%n,fb,'coupe')


! Pas de bilan des apports de r�sidus de culture dans l'inter-coupe
    ...
    ! recalcul des dates de stade dans le calendrier julien si elles sont simulees
      ...
      idebsen = tCal1JAbs(p%ndebsen(1),sc%P_iwater)
...
! dr 10/10/2014 je mets � la place les modifs de bruno
!      write(fb,mes2084) itk%P_msresiduel(p%numcoupe-1),   &
!                        p%masec(sc%AOAS,sc%n) - itk%P_msresiduel(p%numcoupe-1)
! Modif Bruno juin 2014 on calcule le rendement cumul� en fourrage
...
! Fin modif
      ...
      write(fb,mes2039) p%QNplante(sc%AOAS,sc%n),p%CNplante(sc%AOAS) !,sc%n)
          ...
          ! Bilan eau, stress hydrique et azot�
...
!*****************************************************************************************************************************
!EC et BM 08/2012 routine incative ; avant utilis�e pour cultures fauch�es; maitenant on utilise la m�me par d�faut BilanEauCN
    ! 21/08/2012 DR je renomme ce n'est pas tres parlant
!subroutine ecrireBilanRecolteButoir(sc,pg,soil,p,itk)
subroutine ecrireBilanFinCultureFauchee(sc,pg,soil,p,itk)
...
! On ne peut pas changer iplt-P_iwater+1 par nplt car nplt a �t� modifi� en cours de simulation (jour de coupe).
...
!      endif
    ...
    ! DR 15/11/05 NON la ms final est vraiment la recolt�e sans masecneo
    !-- p%masectot=p%masectot+p%masecneoC

!--       CNplanteC(ipl,n)=QNplanteC(ipl,n)/(p%masectot*10)

    ! DR 170106 le QNplante et le CNplante correspondent � la matiere seche reellement recolt�e
...
!TODO : msg bilan intercoupe, � voir si on les conserve ou si on supprime
!      write(fb,mes2082) p%masec(sc%AOAS,sc%n)
!      write(fb,mes2083) p%masec(sc%AOAS,sc%n) - p%msresjaune(sc%AOAS) - p%msneojaune(sc%AOAS),  &
!                        p%msresjaune(sc%AOAS)+p%msneojaune(sc%AOAS)
!
...
! TODO: � voir si on affiche ou pas (issu de intercoupe)
!    ! gel
!      write(fb,mes1166) (1.-p%gel1)*100.,(1.-p%gel2)*100.
!
!      if (p%mortplante == 1) write(fb,mes1167)
...
!subroutine bilanSimulationDefaut(sc,pg,soil,p,itk)    ! DR 19/07/2012 pg n'est pas utilis�
!subroutine bilanSimulationDefaut(sc,soil,p,itk)
! E et B 02/08/2012 changement de nom de cette routine commune � toutes les cultures
...
!    type(Parametres_Generaux_), intent(IN)    :: pg
    ...
    integer :: fb     ! descripteur du fichier bilan dans lequel �crire
    real    :: QH2Oi  !
    real    ::  QH2Of  !
    real    ::  QNO3i  !
    real    ::  QNH4i  !
...
!   Bruno: ajout mai 2012
    real    :: QNexport ! N export�
    real    :: QNrest   ! N restitu�
    real    :: Qmin     ! N mineral
    real    :: QCorgeng ! C associ� au N organis� par l'engrais
    real    :: QNap     ! somme des apports N
    real    :: QCap     ! somme des apports C
    real    :: QNplantfinal   ! quantit� de N dans la plante apr�s r�colte (r�siduel)
    integer :: i
    character(len=45) :: typres
      ...
      ! vitesse de min�ralisation potentielle
        Vp = 0.
        if (sc%tnhc > 0.) Vp = sc%Qminh / sc%tnhc
        Vpannuel = Vp *36525. / sc%Nhumt
        write(fb,mes4070) Vp,Vpannuel
    ! quantit�s d'eau et d'azote sur l'ensemble du profil
        QH2Oi = 0.0         ! TODO: certaines de ces variables sont dans la structure Transit
        QH2Of = 0.0         ! mais ne sont utilis�es que pour les routines bilans,
        QNO3i = 0.0         ! donc replac�es en locales pour l'instant
        QNH4i = 0.0
        QNO3f = 0.0
        QNH4f = 0.0
        do i = 1,5
...
! Elsa 30/07/2012 attention le N denitrifi� est compris dans QNdenit donc nous ajoutons uniquement les emissions de N2O li�es
! � la nitrification, soit Qem_N2O remplac� par Qem_N2Onit. Cette variable QNdeneng est renomm�e QNdenit = azote total denitrifi� engrais + sol

! EC et BM 02/08/2012 les quantit�s de N restitu�es
...
! EC et BM 02/08/2012 les quantit�s export�es et r�siduelles d�pendent du type de plante
     !*******************************
...
! Elsa 30/07/2012 on change l'ordre d'�criture des variables pour homog�n�iser l'�criture des bilans N mineral et organique (1-pools, 2-entrees, 3-sorties)
...
! a faire egalement pour QCrac, QCressuite et QCrogne   et les QN itou
        ...
        QNap = sc%QNresorg + p%QNressuite + p%QNrac + p%QNplantetombe(sc%AOAS) + p%QNrogne !equivalent � sc%QNapp
        ...
        QCap = sc%QCresorg + p%QCressuite + p%QCrac + p%QCplantetombe(sc%AOAS) + p%QCrogne !equivalent � sc%QCapp
...
! Residus de culture
! EC 06/08/2012 pour les cultures fauch�es les r�sidus apport�s au sol sont les racines mortes � la coupe qqsoit le code ressuite
      if (itk%P_codefauche == 1) then
                                              typres = "dead roots     "
      else
          if (p%P_codeplante == "snu") then
...
!             if(itk%P_ressuite=="aucun")           typres = "roots          "
!             if(itk%P_ressuite=="culture")         typres = "whole crop     "
!             if(itk%P_ressuite=="pailles")         typres = "straw   + roots"
!             if(itk%P_ressuite=="chaumes")         typres = "stubble + roots"
!             if(itk%P_ressuite=="pailles_residu9")&
!             &typres = "stubble of residu type 9 + roots "
!             if(itk%P_ressuite=="pailles_residu10")&
!             &typres = "stubble of residu type 10+ roots "
!             if(itk%P_ressuite=="bois")            typres = "prunings       "
!             if(itk%P_ressuite=="racines")         typres = "roots          "
...
! EC et BM 02/08/2012 les bilans sont d�j� calcul�s dans subroutine bilanSimulationDefaut; donc on supprime cette partie et on change le call dans le bilan cultures fauch�es
...
! DR 170106 on met le Qnplante cumul� des coupes
...
!: Arguments
    ...
    integer,              intent(IN)    :: fb       ! descripteur du fichier dans lequel �crire
...
!: Variables locales
    ...
    integer :: k
      ...
      qApports = 0. ! quantit� des apports  ! =   SUM(airg(debut:fin),msc%ASk=airg/=0.)
    ...
    !  sur la simulation mais psc%AS pdt la coupe concern�e,
...
!        write(*,*)'bilans',debut,fin
        do k = debut, fin ! debut et fin de p�riode

        ! s'il y a un apport ce jour l�
          if (sc%airg(k) > 0.0) then

          ! on r�cup�re le jour julien rapport� au calendrier r�el
...
180         format(7x,I2,'-',A3,'-',I4,15x,F7.1)
          ...
          ! on incr�mente le nombre d'apports
            nApports = nApports + 1
          ! on cumule la quantit� totale d'apports
...
!: Variables locales
    ...
    qApports = 0. ! quantit� des apports  ! =   SUM(airg(debut:fin),msc%ASk=anit/=0.)
...
! TODO: n'y aurait-il pas un probl�me en mode CAS ?
          qApports = qApports + sc%anit(k)
          ! DR 09/12/2016 on ecrit aussi les apports par uree dans patures
          ! DR 03/09/2012  voir si ca peut servir dans rapport
          !totapNcoupe(p%numcoupe)= totapNcoupe(p%numcoupe) + sc%anit(k)

        endif

        if(sc%anit_uree(k) > 0.0)then
          jul = tCal1JAbs(k,sc%P_iwater)
          call dates(k,sc%P_iwater,sc%annee(sc%P_iwater),jul,anCours,mois,jour,numMois)
...
!TODO: messages des stades externalis�s pour traduction ! DR 03/09/2012 plus necessaire all in english
! developments stages
      ...
      ! semis
        case('plt')
          if(p%codeinstal == 0) then
            ...
            write(fb,300) car,p%P_stadebbchplt,jour,mois,sc%annee(sc%iplt(p%ipl)),stcum,stcum

          ! DR 07/02/07 si on a activ� decision semis , je l'indique
...
! DR 18/07/2012 je rajoute la  date de germination
      ! germination
        case('ger')
          ...
          if (sc%iplt(p%ipl) /= 0) then
            write(fb,300) car,p%P_stadebbchger,jour,mois,sc%annee(p%igers),      &
                          p%stpltger,stcumger,                     &
      ...
      ! lev�e = d�marrage
        case('lev')
          ...
          if (sc%iplt(p%ipl) /= 0) then
            write(fb,300) car,p%P_stadebbchlev,jour,mois,sc%annee(p%ilevs),      &
        ...
        case('amf')
          ...
          if (p%namf /= 0)then
            ...
            write(fb,300) car,p%P_stadebbchamf,jour,mois,sc%annee(p%iamfs),          &
                          p%P_stlevamf(itk%P_variete),stcum,            &
                          p%lai(sc%AO,p%namf) * p%surf(sc%AO)       &
                        + p%lai(sc%AS,p%namf) * p%surf(sc%AS)
          endif

! ** senescence brute � partir d'une somme de tmp
! --            i=idebsen
...
!          call julien(i,sc%annee(p%ilevs),mois,jour,nummois)
          ...
          if (p%nlax /= 0)then
            ...
            write(fb,300) car,p%P_stadebbchlax,jour,mois,sc%annee(p%ilaxs),     &
        ...
        ! PB - 08/12/2004 - si P_codeperenne=2,P_culturean=2 et pas de r�colte
        !                   lors de la premi�re ann�e alors isens = 0
...
! DR 01/12/2014 je corrige les stades y'a des pbs de recalcule de dates , fallait prendre l'annee du stade
            ...
            if (p%nsen /= 0)then
              ...
              write(fb,300) car,p%P_stadebbchsen,jour,mois,sc%annee(p%isens),     &
...
! DR 01/12/2014 je corrige les stades y'a des pbs de recalcule de dates , fallait prendre l'annee du stade
          ...
          if (p%nlan /= 0)then
            ...
            write(fb,301) car,jour,mois,sc%annee(p%ilans),      &
...
! DR 01/12/2014 je corrige les stades y'a des pbs de recalcule de dates , fallait prendre l'annee du stade
          ...
          if (p%P_codeperenne == 1) then
            write(fb,300) car,p%P_stadebbchflo,jour,mois,sc%annee(p%iflos),      &
...
! DR 01/12/2014 je corrige les stades y'a des pbs de recalcule de dates , fallait prendre l'annee du stade
          ...
          if (p%ndrp /= 0)then
            ...
            write(fb,300) car,p%P_stadebbchdrp,jour,mois,sc%annee(p%idrps),   &
                          stdrp,stcumdrp,                                   &
...
!          if (p%ndebdes /= 0 .and. p%ndebdes <= p%imats) then
          if (p%ndebdes /= 0 .and. p%ndebdes < p%nmat) then
            ...
            i = idess
        ! NB - le 5/10/03 - ajout d'un test sur idess pour �viter pb
            if (idess > 0) then
...
! DR 01/12/2014 je corrige les stades y'a des pbs de recalcule de dates , fallait prendre l'annee du stade
        ...
        ! dans le fichier technique, ces lignes seront utiles
              ...
              write(fb,300) car,p%P_stadebbchdebdes,jour,mois,sc%annee(idess),    &
...
!            if (p%ndebdes /= 0 .and. p%ndebdes < p%imats) then
            ...
            write(fb,300) car,p%P_stadebbchmat,jour,mois,sc%annee(p%imats),      &
                            sominter,stcumdrp,               &
...
! DR 01/12/2014 je corrige les stades y'a des pbs de recalcule de dates , fallait prendre l'annee du stade
        ...
        ! rec
        case('rec')
          p%irecs = max(1,p%irecs)  ! on veut �viter qu'il soit �gal � z�ro
          i = p%irecs
...
! DR 01/12/2014 je corrige les stades y'a des pbs de recalcule de dates , fallait prendre l'annee du stade
          ...
          if (p%nrec /= 0)then
            write(fb,300) car,p%P_stadebbchrec,jour,mois,sc%annee(p%irecs),      &
                          p%stmatrec,stcumdrp,                  &
                          p%lai(sc%AO,p%nrec) * p%surf(sc%AO)     &
                        + p%lai(sc%AS,p%nrec) * p%surf(sc%AS)
          endif
        ! DR 07/02/07 si on a activ� decision semis, on l'indique
...
! DR 01/12/2014 je corrige les stades y'a des pbs de recalcule de dates , fallait prendre l'annee du stade
        ...
        case('cut')
          ...
          call julien(jul,ancours,mois,jour,nummois)
          car = 'cut  day'
          write (fb,301) car,jour,mois,ancours,p%somcour,       &
                         stcum+p%somcour,                       &
        ...
        case('lcu')
          ...
          call julien(jul,ancours,mois,jour,nummois)
          car = 'cut    '
          write(fb,301) car,jour,mois,ancours,                  &
        ...
        ! DR 050808 si on est en enchainement, on a la lev�e de dormance
        ! qui s'est pass�e l'ann�e d'avant
...
! DR 11/04/06 je mets un message au cas o� mat posterieur � rec
...
!     1- la maturit� physiologique (p%P_codrecolte=1)
!     2- la teneur en eau          (p%P_codrecolte=2)
!     3- la teneur en sucre        (p%P_codrecolte=3)
!     4- la teneur en prot�ine     (p%P_codrecolte=4)
...
! ---------------------------------------------------------------
  ...
  ! �talement de la r�colte
    if (itk%P_nbcueille == 2) then
      write(fb,mes422)
      do irecolte = 1,p%nbrecolte-1
        julrec = p%nrecint(irecolte) + sc%P_iwater - 1
        julan = julrec
        ! PB - teauint & nbfrint ne sont pas calcul�s pour les 2 composantes AO/AS
...
!        write(fb,mes3101) irecolte,jour,mois,sc%annee(julan),   &
      ...
      ! on r�int�gre dans pdsfruit et nfruit ce qu'on avait enlev� lors des cueillettes
