...
! de l'int�gration de Stics dans la plateforme ***** de Toulouse o� les bornes d'un
! argument de type tableau doivent �tre connues et d�finies.
    integer,              intent(IN)    :: nbMessages
    type(varying_string), intent(INOUT) :: messages(nbMessages)

!TODO: Tester si messages n'a pas d�j� �t� allou�, auquel cas, d�sallouer pour r�allouer
...
! lecstat
! -------
   messages (139) = 'Error in the file climate file '
...
! -------
    ...
    messages (396) = 'the sowing depth value cannot be < 1.0, it will be forced to 1.0'
    ...
    messages (5600) = 'You can not combine plastic mulching and addition of organic residues (P_nbjres)'
    messages (601) = 'You can not combine plastic mulching and addition of previous crop residues (P_ressuite)'
! USM file
...
! DR 13/08/09
! ******************************************************************
! Introduction des modifs 6.4 � garder pour la version 7.1 Javastics
...
!dr 02/07/2013 correction des messages macroporosit� mauvais
...
! -------
    ...
    messages (157) = 'you have a lai file indicated in the usm , the model will force the lai value by the values of this file !'
...
! --------
    ...
    messages (162) = 'give a reference height weather measurements (P_zr parameter in param_gen.xml) above&
...
!    messages (163) = 'soil evaporation stopped because of the '
    messages (163) = 'soil evaporation stopped because in the layer 1-zesx there is no water to evaporate this day : '
...
!    messages (449) = 'P_stdrpmat to P_stdrpdes (codeforcestdrpdes in the paramv6_gen.xml file)'
    messages (450) = 'You are in a sequence of linked simulations for intercrop (CAS) and the dominant crop&
                     ...
                     &current usm'
! lecture donn�es cycle pr�c�dent
! DR 14/06/2011 modification mise en conformit� param.sol
...
! DR 29/08/2012 j'ajoute ce message d'erreur venant de sommecouche...
    ...
    messages (5160) = 'if artificial drainage activated (macroporosit� must be desactivated and =infil not equal  0)'
    ...
    messages (262) = 'reading param.sti in case of parameter forcing '
   ...
   messages (2100) = '===== the option pasture in the param_newform is not still available and has been desactivated ==='
   messages (183)  = 'the codepalissage (for treillis system) is activated and the values of hautmaxtec or largtec <= 0.0 '
   messages (184)  = 'the codetaille (pruning) is activeated and the value of jultaille <= 0 '
   messages (185)  = 'the codeclaircie (thinning) is activated and the value of juleclair <= 0'
   messages (186)  = 'the codeclaircie (thinning) is activated and the value of nbinfloecl <= 0'
   messages (187)  = 'the codepaillage (plastic mulch) is activated and the value of couvermulchplastique &
                      &or albedomulchplastique <= 0'

   messages (188)  = 'the codeDST (activation fragmentation of soil) is activated and the value parameters <= 0'
   messages (189)  = 'the codeDSTtass (activation compaction of soil sowing/harvest) is activated and the value of parameters <= 0'

   messages (6000)  = 'Different radiation interception for both plants, codetransrad should be the same (e.g. Beer or Radiative)'
