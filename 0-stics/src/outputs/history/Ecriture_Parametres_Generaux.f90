...
! 08/11/2016 paramtres nit et denit de Joel
    ...
    call EnvoyerMsgHistorique('nh4_min (mg N/l)',pg%P_nh4_min)
    call EnvoyerMsgHistorique('pHminden',pg%P_pHminden)
    call EnvoyerMsgHistorique('pHmaxden',pg%P_pHmaxden)
    call EnvoyerMsgHistorique('wfpsc',pg%P_wfpsc)
    call EnvoyerMsgHistorique('tdenitopt_gauss',pg%P_tdenitopt_gauss)
    call EnvoyerMsgHistorique('scale_tdenitopt',pg%P_scale_tdenitopt)
    call EnvoyerMsgHistorique('Kd  (mg N/l)',pg%P_Kd)
    call EnvoyerMsgHistorique('kdesat (d-1)',pg%P_kdesat)
! parametres à options
      if(pg%P_code_vnit==1)then
          call EnvoyerMsgHistorique('Nitrification rate dependence on NH4 : linear')
          call EnvoyerMsgHistorique('P_fnx', pg%P_fnx)
      endif
      if(pg%P_code_vnit==2)then
          call EnvoyerMsgHistorique('Nitrification rate dependence on NH4 : Michaelis_Menten')
          call EnvoyerMsgHistorique('vnitmax (mg N/kg soil)',pg%P_vnitmax)
          call EnvoyerMsgHistorique('Kamm (mg N/l)',pg%P_Kamm)
      endif

      if(pg%P_code_tnit==1) then
          call EnvoyerMsgHistorique('Temperature function for nitrification : linear')
      ...
      if(pg%P_code_tnit==2) then
          call EnvoyerMsgHistorique('Temperature function for nitrification : Gaussian')
          call EnvoyerMsgHistorique('tnitopt_gauss', pg%P_tnitopt_gauss)
          call EnvoyerMsgHistorique('scale_tnitopt', pg%P_scale_tnitopt)
      endif

      if(pg%P_code_rationit==1)then
          call EnvoyerMsgHistorique('Nitrification N20 ratio : constant')
          call EnvoyerMsgHistorique('P_rationit', pg%P_rationit)
      endif
      if(pg%P_code_rationit==2)then
          call EnvoyerMsgHistorique('Nitrification N20 ratio : variable')
      endif

      if(pg%P_code_hourly_wfps_nit==1)then
        call EnvoyerMsgHistorique('Hourly WFPS calculation for nitrification')
      endif

! denit
      if(pg%P_code_pdenit==1)then
        call EnvoyerMsgHistorique('denitrification Soil parameter dependent')
      endif
      if(pg%P_code_pdenit==2)then
        call EnvoyerMsgHistorique('denitrification Soil carbon dependent')
        call EnvoyerMsgHistorique('P_cmin_pdenit', pg%P_cmin_pdenit)
        call EnvoyerMsgHistorique('P_cmax_pdenit', pg%P_cmax_pdenit)
        call EnvoyerMsgHistorique('P_min_pdenit', pg%P_min_pdenit)
        call EnvoyerMsgHistorique('P_max_pdenit', pg%P_max_pdenit)
      endif
      if(pg%P_code_ratiodenit==1)then
        call EnvoyerMsgHistorique('denitrification Denitrification N20 ratio constant')
      endif

      if(pg%P_code_ratiodenit==2)then
        call EnvoyerMsgHistorique('denitrification Denitrification N20 ratio variable')
        call EnvoyerMsgHistorique('P_cmin_pdenit', pg%P_cmin_pdenit)
      endif

      if(pg%P_code_hourly_wfps_denit==1)then
        call EnvoyerMsgHistorique('Hourly WFPS calculation for denitrification')
      endif
...
!    call EnvoyerMsgHistorique('P_voleng', pg%P_voleng(P_engrais(1))
    call EnvoyerMsgHistorique('P_kbio', pg%P_kbio(1))
    call EnvoyerMsgHistorique('P_yres', pg% P_yres(1))
    call EnvoyerMsgHistorique('P_CroCo', pg%P_CroCo(1))
    call EnvoyerMsgHistorique('P_akres', pg%P_akres(1))
    call EnvoyerMsgHistorique('P_bkres', pg%P_bkres(1))
    call EnvoyerMsgHistorique('P_awb', pg%P_awb(1))
    call EnvoyerMsgHistorique('P_bwb', pg%P_bwb(1))
    call EnvoyerMsgHistorique('P_cwb', pg%P_cwb(1))
    call EnvoyerMsgHistorique('P_ahres', pg%P_ahres(1))
    call EnvoyerMsgHistorique('P_bhres', pg%P_bhres(1))
    call EnvoyerMsgHistorique('P_CNresmin', pg%P_CNresmin(1))
    call EnvoyerMsgHistorique('P_CNresmax', pg%P_CNresmax(1))
    call EnvoyerMsgHistorique('P_qmulchruis0', pg%P_qmulchruis0(1))
    call EnvoyerMsgHistorique('P_mouillabilmulch', pg%P_mouillabilmulch(1))
    call EnvoyerMsgHistorique('P_kcouvmlch', pg%P_kcouvmlch(1))
    call EnvoyerMsgHistorique('P_albedomulchresidus', pg%P_albedomulchresidus(1))
