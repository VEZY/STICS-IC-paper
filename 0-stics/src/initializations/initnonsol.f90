...
!     culture : remise � 0 des stades calcul�s
!               reaffectation des sommes entre stades
!               remise � zero des variables plantes
...
subroutine initnonsol(sc,pg,p,itk,soil,sta,t)  ! c supprim�
...
! TODO: harmoniser les param�tres des itin�raires techniques des plantes (paillage, mulch, etc..)
...
! DR 18/06/08 on integre les modifs enchainement (C)ultures (AS)soci�es
...
if (iand(pg%P_flagEcriture,sc%ECRITURE_ECRAN) >0 )print *,' $$$$$$$$$$$$$$ avant lecture cycle prec'
        call Lecture_DonneesCyclePrecedent(sc,pg,p,itk,soil) !,c,sta,t)
      endif


! domi - 04/02/04
! On rajoute des tests pour la compatibilit� entre initialisations et stade de demarrage
      do ipl = 1,sc%P_nbplantes
        if (p(ipl)%P_stade0 == 'snu') then
          if (p(ipl)%P_lai0 /= 0 .or. p(ipl)%P_masec0 /= 0 .or. p(ipl)%P_zrac0 /= 0 .or.  &
            ...
            ! domi - demander � nadine pour P_resperenne0
          endif
        endif
...
! -------------------------------------------------------------
    ...
    ! PB - par d�faut, on se fie au P_codepaillage de la plante principale
    !   if (itk(1)%P_codepaillage /= 2) itk(1)%couvermulch = 0.

    ! PB - 29/07/2009 - D'une part, on lit des albedomulch dans les param�tres g�n�raux. D'autre part, quand le P_codepaillage
    !                   est �gal 2, on lit un albdeomulch dans le fichier technique.
    !                   J'ai fait le choix de toujours transmettre aux routines l'albedomulch associ� � la structure ITK.
    !                   Quand le P_codepaillage n'est pas �gal � 2, alors on affecte dans l'itk la valeur de l'albedomulch
    !                   des param�tres g�n�raux correspondant au P_typemulch choisi dans i'itk.
    !  if (itk(1)%P_codepaillage /= 2) itk(1)%albedomulch = pg%albedomulch(itk(1)%P_typemulch)
    ! DR 24/02/2011 on initialise couvermuch � P_couvermulchplastique pour les paillage plastique
...
! -- variable introuvable -- ndebreserve  = 0
      ...
      ! cas des vernalisantes (herbac�es)
        ...
        ! -- dans le cas de simulations sur 1 seule ann�e, QUE SI LA FIN DE DORMANCE
        ! -- EST CALCULEE PAR BIDABE OU RICHARDSON ET NON SI ELLE EST FORCEE
        ! DR et IGc ces 2 lignes se servent � rien les 2 varaibles sont recalcul�es en dessosu
      ...
      ! affectation des dates de stades observ�s
      ! PB - 15/03/2005 - en enchainement de p�rennes on ne force pas les stades
        ...
        ! DR et ML le 29/01/08 (lendemain de l'anniversaire de notre pr�sident ch�ri)
        ! sachant qu'on a des probl�mes quand on force DRP trop pr�coc�ment sans forcer FLO
        ! c'est-�-dire avant que stlevflo soit accompli, alors on force FLO automatiquement
...
! 25/04/2012 DR et IGC if faut tester si on a activ� l'eclaicissage
...
! DR 14/04/2016
    ...
    ! DR 09/07/07 on iniitialise durvie pour la prairie � voir avec marie
        p(ipl)%durvie(:,:) = 0.
    ! DR 12/08/08 deplac� ici
        p(ipl)%irazo(:,:)  = 0.
        ...
        p(ipl)%masecneo(:)    = 0.
      ! on remet masecneo � z�ro, bonne ou mauvaise id�e ?
      ! TODO: v�rifier l'impact de cet ajout
        p(ipl)%masecneov(:)    = 0.
        ...
        p(ipl)%dltags(:)   = 0.
! 12/08/08 les irazo on ete deplac� plus haut pour reinitialisation
!        irazo(:,n)    = 0.
...
! cet indice n'existe pas :        p(ipl)%surf(AOAS)  = 1.
        ...
        p(ipl)%penfruit(:)    = 0.
        p(ipl)%preserve(:)    = 0.
    ! ML le 25/06/07 mafeuiljaune et consors ne sont pas initialis�s � zero
    ! -- ce qui pose probl�me en cas d'encha�nement donc je les rajoute
        p(ipl)%mafeuiljaune(:) =  0.
        p(ipl)%matigestruc(:) = 0.
        p(ipl)%mareserve(:)   = 0.
        p(ipl)%pfeuilverte(:,0) = 0.

    ! ML le 06/07/07 mafeuilp doit �galement �tre r�initialis�
    ! -- sinon probl�me en cas d'encha�nement donc je le rajoute
        p(ipl)%mafeuilp(:) =  0.

    ! DR 10/07/07 on initialise tursla pour la prairie � voir avec marie
        p(ipl)%tursla(:) = 1.
    ...
    ! DR 12/09/2012 j'ajoute les densite equivalentes en initialisation
        ...
        ! RV: initialisation here. For more details see laidev subroutine.
        if(sc%P_nbplantes.gt.1)then
            p(ipl)%densiteequiv = p(ipl)%densite*2
        else
            p(ipl)%densiteequiv = p(ipl)%densite
        endif
        ...
        p(ipl)%splai(:)       = 1.
        p(ipl)%vmax(:)        = 0.
      ! test sur les param�tres sourcepuits
    ...
    ! offrnodu
      ...
      if (sc%numcult == 1) then
        pg%P_coefb = pg%P_coefb / 100. ! TODO : on modifie un param�tre g�n�ral qui normalement ne doit pas �tre modifi�, � dupliquer !
  ...
  ! DR 16/09/2016 j'ajoute 2 varaibles
      ...
      sc%toteaunoneffic  = 0.
      sc%totir           = 0.
! DR 26/01/2016 ces varaibles avaient ete cree pour macsur , elles etaient reiunitialis�es implicitement par les usms en rotation
! pour Agmip en annees enchain�es c'est plus bon
      sc%drain_from_plt = 0.
...
! DR 12/04/2011 on ajoute les stress etr/etm et tem/etr
   ...
   ! DR 29/12/2014 ajout de cumul de tcult max pour faire moyenne pour Giacomo Agmip Canopy temp
    ...
    ! DR 10/06/2013 en cas d'apport de residus avec beaucoup d'eau on a increment� le nombre d'apports , faut le remettre � 0
    ...
    ! remettre � zero les irrigations de la culture precedente
...
! 13/12/2017 a deplacer plus bas quand on aura instancie codeinstal
...
! --------------------------------------------------------------
    ...
    ! initialisation des stades pour les p�rennes
...
! 13/12/2017 a deplacer ici quand on a instancie codeinstal
        if (p(ipl)%codeinstal <= 0) then
          p(ipl)%rl(:,:)  = 0.
          p(ipl)%drl(:,:) = 0.
        endif
...
!!!!  ACHTUNG !!!!
! 21/03/2016 pour la prairie cette modif pose pb, on la restreint � vigne et pommier
          ...
          if (p(ipl)%P_codelaitr == 1) then
            ...
            sc%tauxcouv(0) = p(1)%P_lai0 ! TODO : � mettre ailleurs que dans une boule plante
          endif
...
!
          ...
          if (p(ipl)%P_masec0 > 0) then
            if (p(ipl)%P_QNplante0 <= 0) then
              ...
              ! seulement � AO et donc quand apres on ralcule innlai on garde innlai =P_innmin
              ! comme Qnplante(0) est calcul� sur les surfaces ( p%QNplante(0,n) = p%QNplante(AO,n) * p%surf(AO) + p%QNplante(AS,n) * p%surf(AS))
              ! on peut affecter la meme valeur � l'ombre et au soleil
              ! faudra enlever la mise � 0 et tester avec une cas
              p(ipl)%QNplante(AO,0) = 0.0
              !p(ipl)%QNplante(AO,0) = p(ipl)%QNplante(AS,0)
              ! TODO: quid de QNplante(AOAS) ?, DR 10/01/2011 il est recalcul� dans cumaoas !
...
! ** domi 25/04/2002 compatibilite unix
          p(ipl)%ulai(sc%aoas,1) = p(ipl)%P_vlaimax
          p(ipl)%ulai(sc%ao,1) = p(ipl)%P_vlaimax
          p(ipl)%ulai(sc%as,1) = p(ipl)%P_vlaimax
        ...
        ! 25/04/2002 domi compatibilite unix (TODO: ???)
          p(ipl)%ulai(sc%aoas,1) = 3.0
          p(ipl)%ulai(sc%ao,1) = 3.0
          p(ipl)%ulai(sc%as,1) = 3.0
          if (drpavantlax) then
            p(ipl)%nflo = 1
            p(ipl)%ndrp = 1
    ...
    ! initialisation des param�tres des courbes de dilution
    ! si option "plante isol�e"
    ! NB le 11/09/05
...
! *----------------------------------------------* c
    ...
    ! Cas n�2:   P_codeinitprec = 1   (reinitialisation)
      ...
      ! Variables AZOTE SOL
        ...
        ! DR 14/06/2017 correction recupere de la branche perenne, c'est Nhum au lieu de Chum
        !sc%Chum(1:sc%nbCouchesSol) = 0.
        sc%Nhum(1:sc%nbCouchesSol) = 0.
...
! dr et ml 22/09/2011 pb de qressuite et qmulch non reinitialises � l'enchainement
        sc%qmulch   = 0.
        ...
        sc%kres(:) = 0.
        sc%NCbio   = 0.
! dr 21/04/2011 ajout de la mise � zero sinon soucis
        sc%Nnondec(:) = 0.
        sc%Cnondec(:) = 0.
        sc%Nmulchnd = 0. !Bruno- nouvelles variables-somme des quantit�s dans chaque r�sidu
        sc%Cmulchnd = 0.
        sc%Nmulchdec = 0.
        sc%Cbmulch = 0.
        sc%Nbmulch = 0.
...
!  write(5588,*)'dans initnonsol apres modif fin iwater',sc%P_iwater,'ifwater',sc%P_ifwater,sc%ifwater_courant,sc%maxwth
    ...
    ! m�morisation des stocks initiaux de MO du sol: C total, N total, Nactif
      sc%Chumt0 = sc%Chumt
      sc%Nhumt0 = sc%Nhumi + sc%Nhuma
      sc%Chuma0  = sc%Chuma
      sc%Nhuma0  = sc%Nhuma
    ! initialisation de la production de CO2
      sc%QCO2sol = 0. ! TODO: � mettre dans initsol !
      sc%QCapp = 0.
      sc%QNapp = 0.
      sc%QCresorg = 0.
      sc%QNresorg = 0.
    ...
    ! 1er cas : profil homog�ne dans chaque couche
      sc%nhe = 0
      do i = 1, sc%nh
        ...
        ! on n'initialise hur, nit et amm que pour la premiere culture et quand on n'enchaine pas
          if (pg%P_codeinitprec == 1 .or. sc%numcult == 1) then
           ...
           ! write(sc%ficdbg,*)'jour1 apres recup.tmp',sc%n, i,iz+sc%nhe,sc%hur(sc%nhe+iz),soil%nit(sc%nhe+iz),soil%no3init(i)
...
! 06/01/2017 on calcule un resmes initial avant tour calul
        sc%nhe = sc%nhe + izmax
      end do


    ! 2eme cas: profil non homog�ne liss� par une fonction spline
      if (sc%NH > 1) then
...
! *********************************************** c
    ...
    ! pas de sol sup�rieur � 10 m, soit 1000 couches de 1 cm
...
! DR 28/02/2011 initialisation de irmulch pour le premier jour passage de etatsurf avant mineral
    ...
    ! DR et FR  24/06/2015 on conditionne l'appel sur l'initialisation des fauches aux cultures fauchees
      ...
      ! dr 11/02/08 on remonte ce test car fauchediff est index� sur la plante
        if (pg%P_codeinitprec == 1 .or. sc%numcult == 1) then
...
! DR 29/04/2013 j'ajoute un compteur pour le calcul forc� de priestley taylor en cas de donnees manquantes
    ...
    ! une fois recup.tmp lu on l'efface comme ca si il n'est pas gener� dans cette simul
    ! il ne peut pas etre reutilis� plus tard
...
!write(444,*)'hur dans initsol',sc%hur
            ...
            !write(2222,*)'initnonsol',sc%numcult,sc%anit_engrais(:)
