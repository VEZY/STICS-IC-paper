...
! * plante, en d�but de cycle.             *
! ******************************************

subroutine initcycle(sc,pg,p,itk,t,i)
  ...
  integer :: i
    ...
    ! changement d'origine si numcult > 1 et enchainement sur des perennes
      if (sc%numcult > 1) then
        ...
        ! pour les vernalisantes, quand on enchaine la plante est d�j� lev�e. On met nlev = 1
          if (p%P_codebfroid == 2) p%nlev = 1


        ! PB - 09/03/2005 - si dormante, on remet somelong � z�ro
        ! (il faudrait v�rifier s'il ne faudrait pas le faire pour les autres plantes aussi)
        ! DR 18/08/06 la Paul c'est des betises d'apres IGC ! si on est en enchainement d'ann�e
