...
USE Stics, only: size_table_soil
...
! DR 15/11/2013 integer, parameter :: size_table_soil = 1000   enlev� et compilation possible grace � l'ajout du module stics qui contient size_table_soil
...
! fichiers relatifs � la plante. ! TODO: est-ce que �a a vraiment sa place ici finalement ?
...
!  parametres plantes
!***********************
! DR 14/09/2012 j'ajoute un param statique = nb_variete et je le mets � 30 maxi, je remplace (12) par(12)
!integer     :: nb_variete_max=12
  ...
  real      :: P_Kmabs1  ! // PARAMETER // Constant of nitrogen uptake by roots for the high affinity system // �mole. cm root-1 // PARPLT // 1
  real      :: P_Kmabs2  ! // PARAMETER // Constant of nitrogen uptake by roots for the low affinity system // �mole. cm root-1 // PARPLT // 1
  ...
  real      :: P_Vmax1  ! // PARAMETER // Rate of slow nitrogen absorption (high affinity system) // �mole cm-1 h-1 // PARPLT // 1
  real      :: P_Vmax2  ! // PARAMETER // Rate of rapid nitrogen absorption (high affinity system) // �mole cm-1 h-1 // PARPLT // 1
  ...
  real      :: P_stlevdno  ! // PARAMETER // phasic duration between emergence and the beginning of nodulation  // degr�s.jours // PARPLT // 1
  real      :: P_stdnofno  ! // PARAMETER // phasic duration between the beginning and the end of nodulation // degree.days // PARPLT // 1
  real      :: P_stfnofvino  ! // PARAMETER // phasic duration between the end of the nodulation and the end of the nodule life   // degr�s.jours // PARPLT // 1
  real      :: P_vitno  ! // PARAMETER // "rate of nodule onset expressed as a proportion of  P_fixmax   per degree day   // nb degr�s.jour-1 // PARPLT // 1
...
!  real      :: P_stflodrp(30)     ! // PARAMETER // phasic duration between FLO and DRP (only for indication) // degr�s.jours // PARPLT // 1
...
! DR 17/09/2012 je mets une dimension de tableau facilement modifiable
  ...
  real,dimension(nb_variete_max)      :: P_stflodrp     ! // PARAMETER // phasic duration between FLO and DRP (only for indication) // degr�s.jours // PARPLT // 1
...
! DR 29/10/07 tranfert de parametres de paramv6 � plt
...
! DR 21/04/2011 P_Nres est renomm� P_Nreserve car on garde Nres pour la quantit� d'azote des residus au meme titre que Cres
...
! **************************************
! param�tres sp�cifiques d'initialisation de l'�tat de la plante
! pour le d�but de la simulation
...
! DR 01/02/2011 je rajoute le njourres
  ...
  integer      :: mortplante
  integer      :: mortplanteN
! DR 09/03/2016 on passe le nb d'eclaircissage possible � 10
  integer      :: neclair(10)
  integer      :: nrogne
  integer      :: neffeuil
  integer      :: jdepuisrec
...
!
  ...
  real      :: precrac(size_table_soil)
  real      :: lracsenz(size_table_soil)
  real      :: drl(0:731,size_table_soil)            ! temporel pour s�nescence (racinaire)
  real      :: somtemprac
  real      :: rl(0:731,size_table_soil)             ! On peut � priori se passer du tableau temporel, � v�rifier
  ...
  real      :: ef_elongation ! // OUTPUT // Shoot elongation effect due to plant shading // 1-P_elongation
  real      :: ulai(0:2,0:731)    ! temporel pour s�nescence    // OUTPUT // Daily relative development unit for LAI // 0-3
  real      :: caljvc
  real      :: somelong
  real      :: somger
...
! DR 11/06/2013 ajout des varaibles de stress racinaires
 ! 11/06/2013 la variable efda devient une varaible de sortie journaliere pour Simtraces
  ...
  real     :: humirac_z(size_table_soil) ! // OUTPUT //profile soil dryness // 0-1
  real     :: efnrac_z(size_table_soil) ! // OUTPUT // effect of minral nitrogen on roots // 0-1
...
! ** variables qui etaientt AO/AS et qu'on a rendu communes
  ...
  real      :: rnet_plant ! // OUTPUT // Net radiation for the plant layer  // Mj.m-2
  real      :: cumrg   ! // OUTPUT // Sum of global radiation during the stage sowing-harvest   // Mj.m-2

! ** NB le 12/11/98 version 4.0
  real      :: irrigprof(0:size_table_soil)
  real      :: stlevdrp0
  real      :: stsenlan0
  real      :: stlaxsen0
  real      :: remobil
  real      :: somcourdrp
  real      :: dtj(0:731)                 ! temporel pour s�nescence (racinaire)    // OUTPUT // Daily efficient temperature for the root growing  // degree C.j-1
...
!      real      :: lrac
  ...
  real      :: reajust
  real      :: upobs(0:731)               ! tableau de for�age
  real      :: stlevamf0
  real      :: stamflax0
...
! DR 09/10/09 j'ajoute somupvtI
  ...
  real      :: racnoy(0:size_table_soil)                 ! TODO: pourquoi 1000 ? racnoy est plutot un tableau temporel � priori.
  ...
  real      :: tdevelop(0:731)            ! temporel pour s�nescence
...
! ** DR le 13/01/06 pour maintient des variables plante si P_codemsfinal=oui
  ...
  real      :: hauteur_nrec
  real      :: largeur_nrec
  ! DR 26/07/2016 on garde le CNplante de la veille pour le calcul des resttutions paturage
...
! ** dr 14022006 pass� en common pour etre recupere dans Ngrain
  real      :: nbgraingel
  real      :: QNplanteCtot
! EC 06/08/2012 ajout d'une variable quantit� de N export�e pour les cultures fauch�es
  real      :: QNplantefauche
...
! dans le cadre du projet PERMED
  ...
  real      :: matuber_rec ! DR 06/04/2012 on garde le matuber � la recolte
  ! 05/05/2015 DR et FR ajout de la variable recolt�e pour les prairies
...
! dr - 08/04/2010
! on a ajout� le codebbch pour ne pas avoir � modifier les fichiers dans la nouvelle version.
  ...
  real :: RsurRUrac     ! // OUTPUT // Fraction of available water reserve (R/RU) over the root profile // 0 � 1
...
!: Les variables AO/AS pourraient �tre indic�es non pas sur un indice fixe �gal � 2
!: En effet, la plante principale n'a que la partie AS, et seule la plante domin�e a AO/AS
!: On pourrait donc cr�er un taille de tableau dynamique selon la dominance de la plante.
...
! DR 07/06/2017 j'ajoute la variable leai qui est la somme de la surface foliaire pour la photosuynthese des feuilles et des epis
  ...
  real      :: durvie(0:2,0:731)            ! temporel pour s�nescence    // OUTPUT // Actual life span of the leaf surface //  degree C
  ...
  real      :: pfeuilverte(0:2,0:731)       ! temporel pour s�nescence    // OUTPUT // Proportion of green leaves in total non-senescent biomass // 0-1


  real      :: CNgrain(0:2)  ! � la base, CNgrain n'est pas un tableau temporel, les calculs ne sont pas pr�vus pour, donc on supprime l'indice temporel    // OUTPUT // Nitrogen concentration of grains  // %
                             ! (0:2,0:731)
                             ! besoin de la valeur pour le jour nrec, on cr�e une variable de stockage pour cette valeur pour supprimer le tableau temporel ?
  real      :: CNplante(0:2) ! � la base, CNplante n'est pas un tableau temporel, les calculs ne sont pas pr�vus pour, donc on supprime l'indice temporel    // OUTPUT // Nitrogen concentration of entire plant  // %
                             ! (0:2,0:731)
                             ! besoin de la valeur pour le jour nrec, on cr�e une variable de stockage pour cette valeur pour supprimer le tableau temporel ?
  real      :: deltai(0:2,0:731)            ! temporel pour s�nescence    // OUTPUT // Daily increase of the green leaf index // m2 leafs.m-2 soil
  real      :: deltai_sencour(0:2,0:731)    ! temporel pour s�nescence (helper variable)
  real      :: dltams_sencour(0:2,0:731)    ! Idem
  real      :: pfeuilverte_sencour(0:2,0:731) ! Idem
  ...
  real      :: dltams(0:2,0:731)            ! temporel pour s�nescence    // OUTPUT // Growth rate of the plant  // t ha-1.j-1
  real      :: dltamsres(0:2)
  real      :: dltaisenat(0:2)
  ...
  real      :: potential_height(0:2) ! Plant height without stress.
  real      :: largeur(0:2)   ! // OUTPUT // Width of the canopy (between interrows) // m
  real      :: varrapforme(0:2)   ! // OUTPUT // Ratio between plant heigth and plant width // ratio
  real      :: inn0(0:2)
  ...
  real      :: laisen(0:2,0:731)            ! pas besoin du tableau temporel(� v�rifier)    // OUTPUT // Leaf area index of senescent leaves // m2 leafs  m-2 soil
  real      :: magrain(0:2,0:731)           ! pas besoin du tableau temporel(� v�rifier mais � 99% s�r)
  ...
  real      :: QNgrain(0:2)  ! � la base, QNgrain n'est pas un tableau temporel, les calculs ne sont pas pr�vus pour, donc on supprime l'indice temporel    // OUTPUT // Amount of nitrogen in harvested organs (grains / fruits) // kg ha-1
                             ! (0:2,0:731)
                             ! besoin de la valeur pour le jour nrec, on cr�e une variable de stockage pour cette valeur pour supprimer le tableau temporel ?
                             ! besoin de la valeur pour le jour ntaille, on cr�e une variable de stockage pour cette valeur pour supprimer le tableau temporel ?
...
! elles sont juste dimensionn�s sur AOAS
! DR 30/04/2021 les dimensions etaient mauvaises
  real      :: H2Orec_percent(0:2) ! // OUTPUT // Water content of harvested organs in percentage// %
  real      :: sucre_percent(0:2)   ! // OUTPUT // Sugar content of fresh harvested organs // % (of fresh weight)
  real      :: huile_percent(0:2)   ! // OUTPUT // Oil content of fresh harvested organs // % (of fresh weight)

  real      :: et0 ! // OUTPUT // eos+eop // mm
!--------
! Quand la plante meurt, on peut la remplacer par du solnu
! puis, dans le cadre d'un enchainement d'ann�es,
! on peut reprendre la plante originelle.
!logical   ::  is_p_originale
!type(Plante_), pointer :: p_originale
! DR 22/04/2016 la variable uniquement calculee dans bilan est � sortir pour le rapport
...
! DR 23/09/2016 on a ajoute 3 variables pour efese representant les valeurs avant coupe
  ...
  real     :: ef_n_w_height(0:2)
  real      :: vmax(0:2)   ! // transitory variable (need it here because can differ between plants in intercrops)
...
! Fonction g�n�rique de lecture des param�tres plante par descripteur de fichier.
!!
!!
subroutine Plante_Lecture_fp(p, file_pointer, method, codeRetour, path, pathplt)
  ...
  select case(method)
    ...
    case DEFAULT !: la m�thode choisie ne correspond � aucune m�thode d�finie => ERREUR
...
! Fonction g�n�rique de lecture des param�tres plante par nom de fichier.
!!
!!
...
! fin record
  ...
  if (codeRetour < 0) then ! Erreur � l'ouverture du fichier
  ...
  if (present(method)) then
    ...
    ! on appelle la lecture du fichier avec le m�thode Stics V6
...
! DR et ML 27/10/09 introduction des stades bbch
      read (fp,*)
      read (fp,*,err = 250) p%P_stadebbchplt
      read (fp,*)
      read (fp,*,err = 250) p%P_stadebbchger
      read (fp,*)
      read (fp,*,err = 250) p%P_stadebbchlev
      read (fp,*)
      read (fp,*,err = 250) p%P_stadebbchamf
      read (fp,*)
      read (fp,*,err = 250) p%P_stadebbchlax
      read (fp,*)
      read (fp,*,err = 250) p%P_stadebbchsen
      read (fp,*)
      read (fp,*,err = 250) p%P_stadebbchflo
      read (fp,*)
      read (fp,*,err = 250) p%P_stadebbchdrp
      read (fp,*)
      read (fp,*,err = 250) p%P_stadebbchnou
      read (fp,*)
      read (fp,*,err = 250) p%P_stadebbchdebdes
      read (fp,*)
      read (fp,*,err = 250) p%P_stadebbchmat
      read (fp,*)
      read (fp,*,err = 250) p%P_stadebbchrec
      read (fp,*)
      read (fp,*,err = 250) p%P_stadebbchfindorm

! TODO : cr�er un param�tre nbVariete ?
!      read (fp,*)
...
!fin du fichier atteinte avant la lecture compl�te des param�tres
...
! * = == = == = == = == = = = > tests de compatibilit� des parametres < = == = == = == = == = == = * c
