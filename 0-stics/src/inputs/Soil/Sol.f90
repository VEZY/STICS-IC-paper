...
! DR 15/11/2013 integer, parameter :: size_table_soil = 1000
...
!  real :: QCO2sol
  real :: vminr  
  real :: aevap  
  real :: amm(size_table_soil)
  real :: nit(0:size_table_soil)  ! pourquoi demarrer à 0 ?
...
! ** parametres sol
  ...
  integer :: izc(5)  
  integer :: ncel  
  integer :: icel(0:size_table_soil)
...
! DR 19/11/2013 Record
      ...
      if (ib0 .ne. 0 ) then                                  ! enabling_record
         ...
         if (ib1 .eq. 0 ) then                                  ! enabling_record
            filepluspath = soil%ficsol                          ! enabling_record
         else                                                   ! enabling_record
            filepluspath = path(1:ib1) // '/' // soil%ficsol ! enabling_record
...
!      open(3,file = soil%ficsol,status = 'old')
 ...
 ! DR 22/08/2012 j'ajoute les 2 parametres soil%P_CsurNsol, soil%P_penterui
      read(3,*,err = 120) soil%P_numsol,soil%P_typsol,soil%P_argi,soil%P_Norg,soil%P_profhum,soil%P_calc,soil%P_pH,    &
                          soil%P_concseuil,soil%P_albedo,soil%P_q0,soil%P_ruisolnu,soil%P_obstarac,soil%P_pluiebat, &
                          soil%P_mulchbat,soil%P_zesx,soil%P_cfes,soil%P_z0solnu,soil%P_CsurNsol, soil%P_penterui
!  115 continue
...
subroutine Sol_Tests(soil,nbCouches,P_ichsl,P_masvolcx,beta_sol)
...
! NB le 15/07/05 activation de P_coderemontcap
    ...
    ! profondeur maximale du sol = 10m
      if (soil%P_obstarac == size_table_soil) soil%P_obstarac = size_table_soil-1
...
subroutine Sol_Ecriture(soil,pg,sc)
...
!                                 soil%P_q0,soil%P_ruisolnu,soil%P_obstarac
      ...
      write(tmp,'(a12,17f12.2)')soil%P_typsol,soil%P_argi,soil%P_Norg,soil%P_profhum,soil%P_calc, &
